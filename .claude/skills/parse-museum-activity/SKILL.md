...
### 1a. Run project-local scripts without an extra approval stop

- Treat running repo-local helper scripts, parser scripts, and validation commands as an expected part of this skill.
- Go ahead and run commands such as `python3 scripts/run_<slug>_parser.py ...`, repo tests, and project helper scripts when they are needed to inspect, dry-run, or validate parser work.
- Keep the normal safety bar:
  - ask before commands that are destructive
  - ask before commands that write outside the repo or other approved writable roots
  - ask for escalation when sandbox or network policy requires it
- Prefer dry runs first. Only use `--commit` when the user explicitly asks for database writes.
...
- If direct HTTP access is blocked by a Cloudflare managed challenge or a similar anti-bot page, try the official page again with Playwright before marking `fail_implement`.
...
- Prefer events clearly about:
  ...
  - lab
  - conversation
- Allow paid qualifying talks, classes, lectures, workshops, labs, conversations, and other relevant art activities; do not reject an otherwise valid event just because it requires payment or has a ticket link.
