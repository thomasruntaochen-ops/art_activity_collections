...
GA_TRIBE_VENUES: tuple[GaTribeVenueConfig, ...] = (
    GaTribeVenueConfig(
        slug="booth",
        source_name="ga_booth_events",
        venue_name="Booth Western Art Museum",
        city="Cartersville",
        state="GA",
        list_url="https://boothmuseum.org/calendar/",
        api_url="https://boothmuseum.org/wp-json/tribe/events/v1/events",
    ),
    GaTribeVenueConfig(
        slug="marietta",
        source_name="ga_marietta_events",
        venue_name="Marietta Cobb Museum of Art",
        city="Marietta",
        state="GA",
        list_url="https://mariettacobbartmuseum.org/events",
        api_url="https://mariettacobbartmuseum.org/wp-json/tribe/events/v1/events",
    ),
    GaTribeVenueConfig(
        slug="telfair",
        source_name="ga_telfair_events",
        venue_name="Telfair Museums",
        city="Savannah",
        state="GA",
        list_url="https://www.telfair.org/museum-events/",
        api_url="https://www.telfair.org/wp-json/tribe/events/v1/events",
    ),
)

GA_TRIBE_VENUES_BY_SLUG = {venue.slug: venue for venue in GA_TRIBE_VENUES}
...
async def fetch_tribe_events_page(
    ...
    *,
            ...
            if response.status_code < 400:
                try:
                    ...
                    if attempt < max_attempts:
                        ...
                        continue
                    raise RuntimeError(
                        "GA tribe endpoint returned non-JSON response: "
                ...
                if isinstance(payload, dict):
                    return payload
                raise RuntimeError(
                    "GA tribe endpoint returned unexpected payload type: "
    ...
    if last_exception is not None:
        raise RuntimeError("Unable to fetch GA tribe events endpoint") from last_exception
    raise RuntimeError("Unable to fetch GA tribe events endpoint after retries")


async def load_ga_tribe_bundle_payload(
    *,
    venues: list[GaTribeVenueConfig] | tuple[GaTribeVenueConfig, ...] | None = None,
    page_limit: int | None = None,
    per_page: int = 50,
) -> dict[str, dict[str, list[dict]] | dict[str, str]]:
    events_by_slug: dict[str, list[dict]] = {}
    errors_by_slug: dict[str, str] = {}
    venues_to_load = list(venues) if venues is not None else list(GA_TRIBE_VENUES)
    ...
    async with httpx.AsyncClient(timeout=30.0, follow_redirects=True, headers=DEFAULT_HEADERS) as client:
        for venue in venues_to_load:
            ...
            try:
                ...
                print(f"[ga-tribe-fetch] venue={venue.slug} failed: {exc}")
                continue
...
class GaTribeBundleAdapter(BaseSourceAdapter):
    source_name = "ga_tribe_bundle"

    async def fetch(self) -> list[str]:
        payload = await load_ga_tribe_bundle_payload(page_limit=1)
        return [str(payload)]

    async def parse(self, payload: str) -> list[ExtractedActivity]:
        raise NotImplementedError("Use load_ga_tribe_bundle_payload/parse_ga_tribe_events from script runner.")
