...
AR_TRIBE_VENUES: tuple[ArTribeVenueConfig, ...] = (
    ArTribeVenueConfig(
        slug="arkmfa",
        source_name="ar_arkmfa_events",
        venue_name="Arkansas Museum of Fine Arts",
        city="Little Rock",
        state="AR",
        list_url="https://arkmfa.org/events/",
        api_url="https://arkmfa.org/wp-json/tribe/events/v1/events",
    ),
    ArTribeVenueConfig(
        slug="windgate",
        source_name="ar_windgate_events",
        venue_name="Windgate Museum of Art",
        city="Conway",
        state="AR",
        list_url="https://www.windgatemuseum.org/events/",
        api_url="https://www.windgatemuseum.org/wp-json/tribe/events/v1/events",
    ),
)

AR_TRIBE_VENUES_BY_SLUG = {venue.slug: venue for venue in AR_TRIBE_VENUES}
...
async def fetch_tribe_events_page(
    ...
    *,
            ...
            if response.status_code < 400:
                try:
                    ...
                    if attempt < max_attempts:
                        ...
                        continue
                    raise RuntimeError(
                        "AR tribe endpoint returned non-JSON response: "
                ...
                if isinstance(payload, dict):
                    return payload
                raise RuntimeError(
                    "AR tribe endpoint returned unexpected payload type: "
    ...
    if last_exception is not None:
        raise RuntimeError("Unable to fetch AR tribe events endpoint") from last_exception
    raise RuntimeError("Unable to fetch AR tribe events endpoint after retries")


async def load_ar_tribe_bundle_payload(
    *,
    venues: list[ArTribeVenueConfig] | tuple[ArTribeVenueConfig, ...] | None = None,
    page_limit: int | None = None,
    per_page: int = 50,
) -> dict[str, dict[str, list[dict]] | dict[str, str]]:
    events_by_slug: dict[str, list[dict]] = {}
    errors_by_slug: dict[str, str] = {}
    venues_to_load = list(venues) if venues is not None else list(AR_TRIBE_VENUES)
    ...
    async with httpx.AsyncClient(timeout=30.0, follow_redirects=True, headers=DEFAULT_HEADERS) as client:
        for venue in venues_to_load:
            ...
            try:
                ...
                print(f"[ar-tribe-fetch] venue={venue.slug} failed: {exc}")
                continue
...
def parse_ar_tribe_events(
    events: list[dict],
    *,
    venue: ArTribeVenueConfig,
...
class ArTribeBundleAdapter(BaseSourceAdapter):
    source_name = "ar_tribe_bundle"

    async def fetch(self) -> list[str]:
        payload = await load_ar_tribe_bundle_payload(page_limit=1)
        return [str(payload)]

    async def parse(self, payload: str) -> list[ExtractedActivity]:
        raise NotImplementedError("Use load_ar_tribe_bundle_payload/parse_ar_tribe_events from script runner.")
...
def _build_row(event_obj: dict, *, venue: ArTribeVenueConfig) -> ExtractedActivity | None:
    ...
    if any(phrase in token_blob for phrase in (" no registration required ", " tickets not required ")):
...
def _should_keep_event(*, title_blob: str, token_blob: str) -> bool:
    if " cancelled " in token_blob or " canceled " in token_blob or " sold out " in token_blob:
        return False
    if any(pattern in title_blob for pattern in HARD_TITLE_REJECT_PATTERNS):
        return False
...
def _infer_activity_type(token_blob: str) -> str:
    if any(
        ...
        for pattern in (" lecture ", " lectures ", " talk ", " talks ", " discussion ", " conversation ")
    ):
        return "lecture"
    return "workshop"
