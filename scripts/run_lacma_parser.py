...
from src.crawlers.adapters.lacma import (  # noqa: E402
    LACMA_EVENTS_URL,
    fetch_lacma_events_page,
...
DEFAULT_CACHE_DIR = Path("data") / "html" / "lacma"
LACMA_SOURCE_URL_PREFIX = "https://www.lacma.org/%"
...
def _write_html_cache(html: str, cache_dir: Path) -> Path:
    ...
    output_path = cache_dir / f"lacma_events_{stamp}.html"
...
def _write_text_dump(
    ...
    *,
    ...
    if source_html_path is not None:
        ...
        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        output_path = dump_dir / f"lacma_events_{stamp}.txt"
    ...
    *,
    ...
    if input_html:
        ...
        if not input_path.exists():
            ...
            raise SystemExit(1)
        print(f"Loading LACMA HTML from file: {input_path}")
        html = input_path.read_text(encoding="utf-8")
        return html, input_path

    try:
        html = await fetch_lacma_events_page(url)
    ...
    if save_html:
        ...
        print(f"Saved raw LACMA HTML cache to: {cache_path}")

    return html, None


def clear_lacma_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(LACMA_SOURCE_URL_PREFIX),
                    Source.name.like("lacma_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(LACMA_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse LACMA events and programs listings. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument("--url", default=LACMA_EVENTS_URL)
    ...
    parser.add_argument(
        ...
        help="Directory used by --save-html (default: data/html/lacma).",
    )
    parser.add_argument(
        "--dump-text",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all LACMA DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.dump_text:
        ...
        print(f"Saved LACMA text dump to: {dump_path}")

    parsed = parse_lacma_events_html(html=html, list_url=args.url)
    ...
    if args.clear:
        deleted = clear_lacma_entries()
        print(
            "Deleted LACMA rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
