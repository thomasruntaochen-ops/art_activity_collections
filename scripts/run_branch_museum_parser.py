...
from src.crawlers.adapters.branch_museum import BRANCH_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.branch_museum import load_branch_museum_payload  # noqa: E402
from src.crawlers.adapters.branch_museum import parse_branch_museum_payload  # noqa: E402
...
def clear_branch_museum_entries() -> dict[str, int]:
    """Also sweeps up the venue's pre-2026-09 rows.

    Branch used to be ingested as the `branch_events` sub-venue of the VA Tribe
    bundle. It was dropped from that bundle on 2026-09-01, which left its old
    rows behind with no parser claiming them -- the URL prefix below covers both
    those and anything this parser writes.
    """
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [("The Branch Museum of Design", "Richmond", "VA")])
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like("https://branchmuseum.org/%"),
                    Source.base_url == "https://branchmuseum.org",
                    Source.name == "branch_events",
                    Source.adapter_type == "branch_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like("https://branchmuseum.org/%")
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fetch and parse The Branch Museum of Design upcoming activities."
    )
    ...
    if args.clear and not args.commit:
        deleted = clear_branch_museum_entries()
        print(
            "Deleted Branch Museum rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_branch_museum_entries()
        print(
            "Deleted Branch Museum rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="branch_museum",
                source_url=BRANCH_EVENTS_URL,
                load_payload=lambda: load_branch_museum_payload(detail_limit=args.detail_limit),
                parse_payload=parse_branch_museum_payload,
                parser_name="run_branch_museum_parser",
                adapter_type="branch_events",
                parsed_label="Branch Museum rows",
