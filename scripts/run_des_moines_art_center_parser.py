...
DMAC_SOURCE_URL_PREFIX = "https://desmoinesartcenter.org/%"


def clear_des_moines_art_center_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [("Des Moines Art Center", "Des Moines", "IA")])
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(DMAC_SOURCE_URL_PREFIX),
                    Source.name.like("des_moines_art_center%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(DMAC_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    ...
    parser.add_argument("--clear", action="store_true", help="Delete scoped Des Moines Art Center rows before commit.")
    args = parser.parse_args()

    if args.clear and not args.commit:
        deleted = clear_des_moines_art_center_entries()
        print(
            "Deleted Des Moines Art Center rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_des_moines_art_center_entries()
        print(
            "Deleted Des Moines Art Center rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
