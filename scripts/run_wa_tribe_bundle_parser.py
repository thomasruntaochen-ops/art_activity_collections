...
from src.crawlers.adapters.wa_tribe_bundle import WA_TRIBE_VENUES  # noqa: E402
from src.crawlers.adapters.wa_tribe_bundle import WA_TRIBE_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.wa_tribe_bundle import get_wa_tribe_source_prefixes  # noqa: E402
from src.crawlers.adapters.wa_tribe_bundle import load_wa_tribe_bundle_payload  # noqa: E402
from src.crawlers.adapters.wa_tribe_bundle import parse_wa_tribe_events  # noqa: E402
...
WA_TRIBE_SOURCE_URL_PREFIXES = get_wa_tribe_source_prefixes()


def clear_wa_tribe_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [(venue.venue_name, venue.city, venue.state) for venue in WA_TRIBE_VENUES],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.in_([venue.list_url for venue in WA_TRIBE_VENUES]),
                    Source.name.like("wa_tribe_%"),
                    Source.name.in_([venue.source_name for venue in WA_TRIBE_VENUES]),
                )
            )
        ).all()

        url_filters = [Activity.source_url.like(f"{prefix}%") for prefix in WA_TRIBE_SOURCE_URL_PREFIXES]
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Washington museums using Tribe events APIs. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(WA_TRIBE_VENUES_BY_SLUG),
        default="all",
        help="Run all WA Tribe venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all WA Tribe bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    selected_venues = (
        list(WA_TRIBE_VENUES)
        if args.venue == "all"
        else [WA_TRIBE_VENUES_BY_SLUG[args.venue]]
    )

    if args.clear and not args.commit:
        deleted = clear_wa_tribe_entries()
        print(
            "Deleted WA Tribe rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_wa_tribe_entries()
        print(
            "Deleted WA Tribe rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    payload = await load_wa_tribe_bundle_payload(
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[wa-tribe-fetch] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any WA Tribe venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed WA Tribe bundle with fetch failures: {failed}")
