...
COLUMBUS_SOURCE_URL_PREFIX = "https://www.columbusmuseum.org/%"


def clear_columbus_museum_of_art_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(COLUMBUS_SOURCE_URL_PREFIX),
                    Source.name.like("columbus_museum_of_art%"),
                    Source.adapter_type == "columbus_museum_of_art_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(COLUMBUS_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    ...
    parser.add_argument(
        "--clear",
        action="store_true",
        help="Delete Columbus Museum of Art rows before repopulating. Requires --commit to repopulate.",
    )
    args = parser.parse_args()

    if args.clear and not args.commit:
        deleted = clear_columbus_museum_of_art_entries()
        print(
            "Deleted Columbus Museum of Art rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_columbus_museum_of_art_entries()
        print(
            "Deleted Columbus Museum of Art rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
