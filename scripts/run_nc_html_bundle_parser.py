...
from src.crawlers.adapters.nc_html_bundle import NC_HTML_VENUES  # noqa: E402
from src.crawlers.adapters.nc_html_bundle import NC_HTML_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.nc_html_bundle import get_nc_html_source_prefixes  # noqa: E402
from src.crawlers.adapters.nc_html_bundle import load_nc_html_bundle_payload  # noqa: E402
from src.crawlers.adapters.nc_html_bundle import parse_nc_html_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse North Carolina museums using HTML/detail-page parsing. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(NC_HTML_VENUES_BY_SLUG),
        default="all",
        help="Run all NC HTML bundle venues or one venue slug.",
    )
    parser.add_argument(
        "--commit",
        action="store_true",
    ...
    selected_venues = (
        list(NC_HTML_VENUES)
        if args.venue == "all"
        else [NC_HTML_VENUES_BY_SLUG[args.venue]]
    )
    ...
    for venue in selected_venues:
        targets.append(
            TargetRunSpec(
                name=f"nc_html_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_nc_html_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_nc_html_bundle_parser",
