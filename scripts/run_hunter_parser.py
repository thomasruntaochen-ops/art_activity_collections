...
from src.crawlers.adapters.hunter import HUNTER_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.hunter import load_hunter_payload  # noqa: E402
from src.crawlers.adapters.hunter import parse_hunter_payload  # noqa: E402
...
HUNTER_SOURCE_URL_PREFIX = "https://www.huntermuseum.org/%"


def clear_hunter_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(HUNTER_SOURCE_URL_PREFIX),
                    Source.name.like("hunter_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(HUNTER_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Hunter Museum of American Art event listings. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Hunter Museum of American Art DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_hunter_entries()
        print(
            "Deleted Hunter Museum rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_hunter_entries()
        print(
            "Deleted Hunter Museum rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="hunter",
                source_url=HUNTER_EVENTS_URL,
                load_payload=lambda: load_hunter_payload(page_limit=args.page_limit),
                parse_payload=parse_hunter_payload,
                parser_name="run_hunter_parser",
                adapter_type="hunter_events",
                parsed_label="Hunter Museum rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Hunter Museum of American Art"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_hunter_parser",
            source_url=HUNTER_EVENTS_URL,
            details={"source": "Hunter Museum of American Art"},
        ),
    )
