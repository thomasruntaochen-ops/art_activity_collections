...
from src.crawlers.adapters.alexandria_museum_of_art import ALEXANDRIA_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.alexandria_museum_of_art import load_alexandria_museum_of_art_payload  # noqa: E402
from src.crawlers.adapters.alexandria_museum_of_art import parse_alexandria_museum_of_art_payload  # noqa: E402
...
ALEXANDRIA_SOURCE_URL_PREFIX = "https://themuseum.org/%"


def clear_alexandria_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(ALEXANDRIA_SOURCE_URL_PREFIX),
                    Source.name.in_(["themuseum.org", "alexandria_museum_of_art_events"]),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(ALEXANDRIA_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Alexandria Museum of Art events. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument("--url", default=ALEXANDRIA_EVENTS_URL)
    parser.add_argument(
        "--commit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Alexandria Museum of Art DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_alexandria_entries()
        print(
            "Deleted Alexandria Museum of Art rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_alexandria_entries()
        print(
            "Deleted Alexandria rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    payload = await load_alexandria_museum_of_art_payload(url=args.url)
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="alexandria_museum_of_art",
                source_url=args.url,
                load_payload=_load_payload,
                parse_payload=parse_alexandria_museum_of_art_payload,
                parser_name="run_alexandria_museum_of_art_parser",
                adapter_type="alexandria_museum_of_art_events",
                parsed_label="alexandria rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Alexandria Museum of Art"},
            )
        ],
        commit=args.commit,
    )
