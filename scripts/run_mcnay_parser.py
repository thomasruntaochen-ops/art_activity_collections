...
def _source_url_prefix(url: str) -> str:
    ...
    return MCNAY_EVENTS_URL


def clear_mcnay_entries(source_url: str = MCNAY_EVENTS_URL) -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [(MCNAY_VENUE_NAME, MCNAY_CITY, MCNAY_STATE)])
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url == source_url,
                    Source.name == "mcnay_events",
                    Source.adapter_type == "mcnay_events",
                )
            )
        ).all()
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete existing McNay DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_mcnay_entries(args.url)
        print(
            "Deleted McNay rows: "
    ...
    if args.commit:
        ...
        if args.clear:
            deleted = clear_mcnay_entries(args.url)
            print(
                "Deleted McNay rows before repopulation: "
                f"activity_tags={deleted['activity_tags']}, "
                f"activities={deleted['activities']}, "
                f"ingestion_runs={deleted['ingestion_runs']}, "
                f"sources={deleted['sources']}"
