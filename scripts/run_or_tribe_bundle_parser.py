...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Oregon museums using Tribe events APIs. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(OR_TRIBE_VENUES_BY_SLUG),
        default="all",
        help="Run all OR Tribe venues or one venue slug.",
    )
    ...
    selected_venues = (
        list(OR_TRIBE_VENUES)
        if args.venue == "all"
        else [OR_TRIBE_VENUES_BY_SLUG[args.venue]]
    )
    ...
    payload = await load_or_tribe_bundle_payload(
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[or-tribe-fetch] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any OR Tribe venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed OR Tribe bundle with fetch failures: {failed}")
