...
from src.crawlers.adapters.museum_of_wisconsin_art import MOWA_CITY  # noqa: E402
from src.crawlers.adapters.museum_of_wisconsin_art import MOWA_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.museum_of_wisconsin_art import MOWA_STATE  # noqa: E402
from src.crawlers.adapters.museum_of_wisconsin_art import MOWA_VENUE_NAME  # noqa: E402
...
MOWA_SOURCE_URL_PREFIX = "https://wisconsinart.org/"


def clear_museum_of_wisconsin_art_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [(MOWA_VENUE_NAME, MOWA_CITY, MOWA_STATE)])
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url == MOWA_EVENTS_URL,
                    Source.name == "museum_of_wisconsin_art_events",
                    Source.adapter_type == "museum_of_wisconsin_art_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(f"{MOWA_SOURCE_URL_PREFIX}%")
        if source_ids:
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete Museum of Wisconsin Art DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_museum_of_wisconsin_art_entries()
        print(
            "Deleted Museum of Wisconsin Art rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_museum_of_wisconsin_art_entries()
        print(
            "Deleted Museum of Wisconsin Art rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
