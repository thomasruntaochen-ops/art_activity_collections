...
from src.crawlers.adapters.buffalo_akg import BUFFALO_AKG_FIND_EVENT_URL  # noqa: E402
from src.crawlers.adapters.buffalo_akg import load_buffalo_akg_payload  # noqa: E402
from src.crawlers.adapters.buffalo_akg import parse_buffalo_akg_payload  # noqa: E402
...
BUFFALO_AKG_SOURCE_URL_PREFIX = "https://buffaloakg.org/%"


def clear_buffalo_akg_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(BUFFALO_AKG_SOURCE_URL_PREFIX),
                    Source.name.like("buffalo_akg_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(BUFFALO_AKG_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Buffalo AKG Art Museum event pages. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Buffalo AKG DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_buffalo_akg_entries()
        print(
            "Deleted Buffalo AKG rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_buffalo_akg_entries()
        print(
            "Deleted Buffalo AKG rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="buffalo_akg",
                source_url=BUFFALO_AKG_FIND_EVENT_URL,
                load_payload=lambda: load_buffalo_akg_payload(page_limit=args.page_limit),
                parse_payload=parse_buffalo_akg_payload,
                parser_name="run_buffalo_akg_parser",
                adapter_type="buffalo_akg_events",
                parsed_label="Buffalo AKG rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Buffalo AKG Art Museum"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_buffalo_akg_parser",
            source_url=BUFFALO_AKG_FIND_EVENT_URL,
            details={"source": "Buffalo AKG Art Museum"},
        ),
    )
