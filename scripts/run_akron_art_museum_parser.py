...
from src.crawlers.adapters.akron_art_museum import AKRON_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.akron_art_museum import load_akron_art_museum_payload  # noqa: E402
from src.crawlers.adapters.akron_art_museum import parse_akron_art_museum_payload  # noqa: E402
...
def clear_akron_art_museum_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [("Akron Art Museum", "Akron", "OH")])
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like("https://akronartmuseum.org/%"),
                    Source.name == "akron_art_museum_events",
                    Source.adapter_type == "akron_art_museum_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like("https://akronartmuseum.org/%")
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch and parse Akron Art Museum upcoming activities.")
    parser.add_argument("--commit", action="store_true")
    parser.add_argument(
        "--clear",
        action="store_true",
        help=(
            "Delete all Akron Art Museum DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_akron_art_museum_entries()
        print(
            "Deleted Akron Art Museum rows: "
    ...
    payload = await load_akron_art_museum_payload()
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_akron_art_museum_entries()
        print(
            "Deleted Akron Art Museum rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="akron_art_museum",
                source_url=AKRON_EVENTS_URL,
                load_payload=lambda: asyncio.sleep(0, result=payload),
                parse_payload=parse_akron_art_museum_payload,
                parser_name="run_akron_art_museum_parser",
                adapter_type="akron_art_museum_events",
                parsed_label="Akron Art Museum rows",
