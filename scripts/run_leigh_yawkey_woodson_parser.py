...
def clear_leigh_yawkey_woodson_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [("Leigh Yawkey Woodson Art Museum", "Wausau", "WI")])
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like("https://www.lywam.org/%"),
                    Source.name == "leigh_yawkey_woodson_events",
                    Source.adapter_type == "leigh_yawkey_woodson_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like("https://www.lywam.org/%")
        if source_ids:
...
async def main() -> None:
    ...
    if args.clear and not args.commit:
        deleted = clear_leigh_yawkey_woodson_entries()
        print(
            "Deleted Leigh Yawkey Woodson rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_leigh_yawkey_woodson_entries()
        print(
            "Deleted Leigh Yawkey Woodson rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
