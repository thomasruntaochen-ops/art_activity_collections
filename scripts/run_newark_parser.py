...
from src.crawlers.adapters.newark import NEWARK_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.newark import load_newark_events_payload  # noqa: E402
from src.crawlers.adapters.newark import parse_newark_events_payload  # noqa: E402
...
NEWARK_SOURCE_URL_PREFIX = "https://newarkmuseumart.org/%"


def clear_newark_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(NEWARK_SOURCE_URL_PREFIX),
                    Source.name.like("newark_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(NEWARK_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Newark Museum of Art events. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Newark Museum DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_newark_entries()
        print(
            "Deleted Newark rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    run_specs = [
        TargetRunSpec(
            name="newark_events",
            source_url=NEWARK_EVENTS_URL,
            load_payload=load_newark_events_payload,
            parse_payload=parse_newark_events_payload,
            parser_name="run_newark_parser",
            adapter_type="newark_events",
            parsed_label="Newark rows",
