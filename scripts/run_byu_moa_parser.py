...
from src.crawlers.adapters.byu_moa import BYU_MOA_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.byu_moa import load_byu_moa_payload  # noqa: E402
from src.crawlers.adapters.byu_moa import parse_byu_moa_payload  # noqa: E402
...
BYU_SOURCE_URL_PREFIX = "https://moa.byu.edu/%"


def clear_byu_moa_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(BYU_SOURCE_URL_PREFIX),
                    Source.name.like("byu_moa_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(BYU_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse BYU Museum of Art events from the official MOA events page. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all BYU MOA DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_byu_moa_entries()
        print(
            "Deleted BYU MOA rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_byu_moa_entries()
        print(
            "Deleted BYU MOA rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="byu_moa_events",
                source_url=BYU_MOA_EVENTS_URL,
                load_payload=lambda: asyncio.sleep(0, result=payload),
                parse_payload=parse_byu_moa_payload,
                parser_name="run_byu_moa_parser",
                adapter_type="byu_moa_events",
                parsed_label="BYU MOA rows",
