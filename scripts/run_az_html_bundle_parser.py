...
from src.crawlers.adapters.az_html_bundle import AZ_HTML_VENUES  # noqa: E402
from src.crawlers.adapters.az_html_bundle import AZ_HTML_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.az_html_bundle import get_az_html_source_prefixes  # noqa: E402
from src.crawlers.adapters.az_html_bundle import load_az_html_bundle_payload  # noqa: E402
from src.crawlers.adapters.az_html_bundle import parse_az_html_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Arizona museums using HTML and Playwright parsing. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(AZ_HTML_VENUES_BY_SLUG),
        default="all",
        help="Run all AZ HTML venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all AZ HTML bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    for venue in selected_venues:
        targets.append(
            TargetRunSpec(
                name=f"az_html_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_az_html_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_az_html_bundle_parser",
