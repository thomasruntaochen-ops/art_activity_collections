...
from src.crawlers.adapters.de_bundle import DE_VENUES  # noqa: E402
from src.crawlers.adapters.de_bundle import DE_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.de_bundle import get_de_source_prefixes  # noqa: E402
from src.crawlers.adapters.de_bundle import load_de_bundle_payload  # noqa: E402
from src.crawlers.adapters.de_bundle import parse_de_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Delaware museum activities from a mixed API/HTML bundle. "
            "Print parsed rows, and optionally commit to MySQL."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(DE_VENUES_BY_SLUG),
        default="all",
        help="Run all Delaware bundle venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Delaware bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    payload = await load_de_bundle_payload(
    ...
    for venue in selected_venues:
        ...
        targets.append(
            TargetRunSpec(
                name=f"de_bundle_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload_by_slug.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_de_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_de_bundle_parser",
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[de-bundle] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any DE bundle venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed DE bundle with fetch failures: {failed}")
