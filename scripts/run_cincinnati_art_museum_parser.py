...
from src.crawlers.adapters.cincinnati_art_museum import CINCINNATI_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.cincinnati_art_museum import load_cincinnati_art_museum_payload  # noqa: E402
from src.crawlers.adapters.cincinnati_art_museum import parse_cincinnati_art_museum_payload  # noqa: E402
...
CINCINNATI_SOURCE_URL_PREFIX = "https://www.cincinnatiartmuseum.org/%"


def clear_cincinnati_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [("Cincinnati Art Museum", "Cincinnati", "OH")])

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(CINCINNATI_SOURCE_URL_PREFIX),
                    Source.name == "cincinnati_art_museum_events",
                    Source.adapter_type == "cincinnati_art_museum_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(CINCINNATI_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch and parse Cincinnati Art Museum upcoming activities.")
    parser.add_argument("--commit", action="store_true")
    parser.add_argument("--clear", action="store_true")
    args = parser.parse_args()

    if args.clear and not args.commit:
        deleted = clear_cincinnati_entries()
        print(
            "Deleted Cincinnati rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_cincinnati_entries()
        print(
            "Deleted Cincinnati rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    payload = await load_cincinnati_art_museum_payload()
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="cincinnati_art_museum",
                source_url=CINCINNATI_EVENTS_URL,
                load_payload=lambda: asyncio.sleep(0, result=payload),
                parse_payload=parse_cincinnati_art_museum_payload,
                parser_name="run_cincinnati_art_museum_parser",
                adapter_type="cincinnati_art_museum_events",
                parsed_label="Cincinnati Art Museum rows",
