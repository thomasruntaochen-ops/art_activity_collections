...
from src.crawlers.adapters.elmuseo import ELMUSEO_EVENTS_LIST_URL  # noqa: E402
from src.crawlers.adapters.elmuseo import load_elmuseo_payload  # noqa: E402
from src.crawlers.adapters.elmuseo import parse_elmuseo_payload  # noqa: E402
...
ELMUSEO_SOURCE_URL_PREFIX = "https://elmuseo.org/%"


def clear_elmuseo_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(ELMUSEO_SOURCE_URL_PREFIX),
                    Source.name.like("elmuseo_%"),
                )
            )
        ).all()
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse El Museo del Barrio event pages. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all El Museo DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_elmuseo_entries()
        print(
            "Deleted El Museo rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_elmuseo_entries()
        print(
            "Deleted El Museo rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="elmuseo",
                source_url=ELMUSEO_EVENTS_LIST_URL,
                load_payload=lambda: load_elmuseo_payload(page_limit=args.page_limit),
                parse_payload=parse_elmuseo_payload,
                parser_name="run_elmuseo_parser",
                adapter_type="elmuseo_events",
                parsed_label="El Museo rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "El Museo del Barrio"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_elmuseo_parser",
            source_url=ELMUSEO_EVENTS_LIST_URL,
            details={"source": "El Museo del Barrio"},
        ),
    )
