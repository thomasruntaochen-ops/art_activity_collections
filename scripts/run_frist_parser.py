...
from src.crawlers.adapters.frist import FRIST_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.frist import load_frist_payload  # noqa: E402
from src.crawlers.adapters.frist import parse_frist_payload  # noqa: E402
...
FRIST_SOURCE_URL_PREFIX = "https://fristartmuseum.org/%"


def clear_frist_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(FRIST_SOURCE_URL_PREFIX),
                    Source.name.like("frist_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(FRIST_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Frist Art Museum event listings. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Frist Art Museum DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_frist_entries()
        print(
            "Deleted Frist Art Museum rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_frist_entries()
        print(
            "Deleted Frist Art Museum rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="frist",
                source_url=FRIST_EVENTS_URL,
                load_payload=lambda: load_frist_payload(page_limit=args.page_limit),
                parse_payload=parse_frist_payload,
                parser_name="run_frist_parser",
                adapter_type="frist_events",
                parsed_label="Frist Art Museum rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Frist Art Museum"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_frist_parser",
            source_url=FRIST_EVENTS_URL,
            details={"source": "Frist Art Museum"},
        ),
    )
