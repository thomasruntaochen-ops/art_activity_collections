...
from src.crawlers.adapters.crow import (  # noqa: E402
    CROW_EVENTS_URL,
    load_crow_payload,
    parse_crow_payload,
)
...
DEFAULT_CACHE_DIR = Path("data") / "html" / "crow"
...
def _write_html_cache(payload: dict[str, object], cache_dir: Path) -> Path:
    ...
    output_path = cache_dir / f"crow_events_{stamp}.json"
...
def _write_text_dump(payload: dict[str, object], dump_dir: Path) -> Path:
    ...
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    output_path = dump_dir / f"crow_events_{stamp}.txt"
    ...
    *,
    ...
    if input_json:
        ...
        if not input_path.exists():
            ...
            raise SystemExit(1)
        print(f"Loading Crow payload from file: {input_path}")
        return json.loads(input_path.read_text(encoding="utf-8"))

    try:
        payload = await load_crow_payload(url)
    ...
    if save_html:
        ...
        print(f"Saved raw Crow payload cache to: {cache_path}")
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Crow Museum event listings. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument("--url", default=CROW_EVENTS_URL)
    ...
    parser.add_argument(
        ...
        help="Directory used by --save-html (default: data/html/crow).",
    )
    parser.add_argument(
        "--dump-text",
        action="store_true",
    ...
    if args.dump_text:
        ...
        print(f"Saved Crow text dump to: {dump_path}")

    parsed = parse_crow_payload(payload)
