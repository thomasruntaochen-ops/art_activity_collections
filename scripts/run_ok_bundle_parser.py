...
from src.crawlers.adapters.ok_bundle import OK_VENUES  # noqa: E402
from src.crawlers.adapters.ok_bundle import OK_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.ok_bundle import get_ok_source_prefixes  # noqa: E402
from src.crawlers.adapters.ok_bundle import load_ok_bundle_payload  # noqa: E402
from src.crawlers.adapters.ok_bundle import parse_ok_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Oklahoma museum activities from mixed official sources. "
            "Print parsed rows, and optionally commit to MySQL."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(OK_VENUES_BY_SLUG),
        default="all",
        help="Run all Oklahoma venues or one venue slug.",
    )
    parser.add_argument(
        "--start-date",
        default=None,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all OK bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    payload = await load_ok_bundle_payload(venues=selected_venues)
    ...
    for venue in selected_venues:
        ...
        targets.append(
            TargetRunSpec(
                name=f"ok_bundle_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload_by_slug.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_ok_events(
                    venue_payload,
                    venue=selected_venue,
                    start_date=args.start_date,
                ),
                parser_name="run_ok_bundle_parser",
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[ok-bundle] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any OK bundle venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed OK bundle with fetch failures: {failed}")
