...
from src.crawlers.adapters.nga import NGA_CALENDAR_BASE_URL  # noqa: E402
from src.crawlers.adapters.nga import NGA_VENUE_NAME  # noqa: E402
from src.crawlers.adapters.nga import load_nga_calendar_payload  # noqa: E402
from src.crawlers.adapters.nga import parse_nga_calendar_payload  # noqa: E402
...
NGA_SOURCE_URL_PREFIX = "https://www.nga.gov/calendar%"


def clear_nga_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [(NGA_VENUE_NAME, "Washington", "DC")],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(NGA_SOURCE_URL_PREFIX),
                    Source.name.like("nga_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(NGA_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all National Gallery DB rows (activity_tags, activities, ingestion_runs, "
            "sources). If used without --commit, the script exits after deletion."
        ),
    )
    args = parser.parse_args()

    if args.clear and not args.commit:
        deleted = clear_nga_entries()
        print(
            "Deleted NGA rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_nga_entries()
        print(
            "Deleted NGA rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name=f"nga_{args.audience}",
                source_url=NGA_CALENDAR_BASE_URL,
                load_payload=lambda: load_nga_calendar_payload(
                    audience=args.audience,
                    timeout_ms=args.timeout_ms,
                ),
                parse_payload=parse_nga_calendar_payload,
                parser_name="run_nga_parser",
                adapter_type="nga_calendar",
                parsed_label="nga rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"venue": NGA_VENUE_NAME, "audience": args.audience},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_nga_parser",
            source_url=NGA_CALENDAR_BASE_URL,
            details={"venue": NGA_VENUE_NAME, "audience": args.audience},
        ),
    )
