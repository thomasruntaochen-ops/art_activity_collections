...
def clear_canton_museum_of_art_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [("Canton Museum of Art", "Canton", "OH")])
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like("https://www.cantonart.org/%"),
                    Source.name == "canton_museum_of_art_events",
                    Source.adapter_type == "canton_museum_of_art_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like("https://www.cantonart.org/%")
        if source_ids:
...
async def main() -> None:
    ...
    if args.clear and not args.commit:
        deleted = clear_canton_museum_of_art_entries()
        print(
            "Deleted Canton Museum of Art rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_canton_museum_of_art_entries()
        print(
            "Deleted Canton Museum of Art rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
