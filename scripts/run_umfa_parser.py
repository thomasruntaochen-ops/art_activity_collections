...
from src.crawlers.adapters.umfa import UMFA_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.umfa import load_umfa_payload  # noqa: E402
from src.crawlers.adapters.umfa import parse_umfa_payload  # noqa: E402
...
UMFA_SOURCE_URL_PREFIX = "https://umfa.utah.edu/%"


def clear_umfa_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(UMFA_SOURCE_URL_PREFIX),
                    Source.name.like("umfa_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(UMFA_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Utah Museum of Fine Arts events from the Tribe events API. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all UMFA DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    payload = await load_umfa_payload(page_limit=args.page_limit)
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_umfa_entries()
        print(
            "Deleted UMFA rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="umfa_events",
                source_url=UMFA_EVENTS_URL,
                load_payload=lambda: asyncio.sleep(0, result=payload),
                parse_payload=parse_umfa_payload,
                parser_name="run_umfa_parser",
                adapter_type="umfa_events",
                parsed_label="UMFA rows",
