...
from src.crawlers.adapters.walters import WALTERS_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.walters import load_walters_events_payload  # noqa: E402
from src.crawlers.adapters.walters import parse_walters_events_payload  # noqa: E402
...
WALTERS_SOURCE_URL_PREFIX = "https://thewalters.org/%"


def clear_walters_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(WALTERS_SOURCE_URL_PREFIX),
                    Source.name.like("walters_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(WALTERS_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Walters Art Museum events from the Tribe events API. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Walters DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_walters_entries()
        print(
            "Deleted Walters rows: "
    ...
    payload = await load_walters_events_payload(page_limit=args.page_limit)
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_walters_entries()
        print(
            "Deleted Walters rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="walters_events",
                source_url=WALTERS_EVENTS_URL,
                load_payload=lambda: asyncio.sleep(0, result=payload),
                parse_payload=parse_walters_events_payload,
                parser_name="run_walters_parser",
                adapter_type="walters_events",
                parsed_label="Walters rows",
