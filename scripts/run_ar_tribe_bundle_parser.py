...
from src.crawlers.adapters.ar_tribe_bundle import AR_TRIBE_VENUES  # noqa: E402
from src.crawlers.adapters.ar_tribe_bundle import AR_TRIBE_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.ar_tribe_bundle import get_ar_tribe_source_prefixes  # noqa: E402
from src.crawlers.adapters.ar_tribe_bundle import load_ar_tribe_bundle_payload  # noqa: E402
from src.crawlers.adapters.ar_tribe_bundle import parse_ar_tribe_events  # noqa: E402
...
AR_TRIBE_SOURCE_URL_PREFIXES = get_ar_tribe_source_prefixes()


def clear_ar_tribe_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [(venue.venue_name, venue.city, venue.state) for venue in AR_TRIBE_VENUES],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.in_([venue.list_url for venue in AR_TRIBE_VENUES]),
                    Source.name.like("ar_%_events"),
                    Source.name.in_([venue.source_name for venue in AR_TRIBE_VENUES]),
                )
            )
        ).all()

        url_filters = [Activity.source_url.like(f"{prefix}%") for prefix in AR_TRIBE_SOURCE_URL_PREFIXES]
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Arkansas museums using Tribe events APIs. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(AR_TRIBE_VENUES_BY_SLUG),
        default="all",
        help="Run all AR Tribe venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all AR Tribe bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
            "If used without --commit, the script exits after deletion."
        ),
    )
    args = parser.parse_args()

    selected_venues = list(AR_TRIBE_VENUES) if args.venue == "all" else [AR_TRIBE_VENUES_BY_SLUG[args.venue]]

    if args.clear and not args.commit:
        deleted = clear_ar_tribe_entries()
        print(
            "Deleted AR Tribe rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_ar_tribe_entries()
        print(
            "Deleted AR Tribe rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    payload = await load_ar_tribe_bundle_payload(
    ...
    for venue in selected_venues:
        ...
        targets.append(
            TargetRunSpec(
                name=f"ar_tribe_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result={"events": events_by_slug.get(slug) or []}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_ar_tribe_events(
                    venue_payload.get("events") or [],
                    venue=selected_venue,
                ),
                parser_name="run_ar_tribe_bundle_parser",
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[ar-tribe-fetch] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any AR Tribe venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed AR Tribe bundle with fetch failures: {failed}")
