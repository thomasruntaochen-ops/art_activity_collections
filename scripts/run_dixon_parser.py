...
from src.crawlers.adapters.dixon import DIXON_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.dixon import load_dixon_payload  # noqa: E402
from src.crawlers.adapters.dixon import parse_dixon_payload  # noqa: E402
...
DIXON_SOURCE_URL_PREFIX = "https://www.dixon.org/%"


def clear_dixon_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(DIXON_SOURCE_URL_PREFIX),
                    Source.name.like("dixon_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(DIXON_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Dixon Gallery & Gardens event listings. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Dixon Gallery & Gardens DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_dixon_entries()
        print(
            "Deleted Dixon Gallery & Gardens rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_dixon_entries()
        print(
            "Deleted Dixon Gallery & Gardens rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="dixon",
                source_url=DIXON_EVENTS_URL,
                load_payload=lambda: load_dixon_payload(page_limit=args.page_limit),
                parse_payload=parse_dixon_payload,
                parser_name="run_dixon_parser",
                adapter_type="dixon_events",
                parsed_label="Dixon Gallery & Gardens rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Dixon Gallery & Gardens"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_dixon_parser",
            source_url=DIXON_EVENTS_URL,
            details={"source": "Dixon Gallery & Gardens"},
        ),
    )
