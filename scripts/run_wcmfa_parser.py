...
from src.crawlers.adapters.wcmfa import WCMFA_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.wcmfa import load_wcmfa_events_payload  # noqa: E402
from src.crawlers.adapters.wcmfa import parse_wcmfa_events_payload  # noqa: E402
...
WCMFA_SOURCE_URL_PREFIX = "https://wcmfa.org/%"


def clear_wcmfa_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(WCMFA_SOURCE_URL_PREFIX),
                    Source.name.like("wcmfa_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(WCMFA_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Washington County Museum of Fine Arts events from the Tribe events API. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all WCMFA DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    payload = await load_wcmfa_events_payload(page_limit=args.page_limit)
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_wcmfa_entries()
        print(
            "Deleted WCMFA rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="wcmfa_events",
                source_url=WCMFA_EVENTS_URL,
                load_payload=lambda: asyncio.sleep(0, result=payload),
                parse_payload=parse_wcmfa_events_payload,
                parser_name="run_wcmfa_parser",
                adapter_type="wcmfa_events",
                parsed_label="WCMFA rows",
