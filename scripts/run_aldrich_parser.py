...
from src.crawlers.adapters.aldrich import ALDRICH_CALENDAR_URL  # noqa: E402
from src.crawlers.adapters.aldrich import load_aldrich_calendar_payload  # noqa: E402
from src.crawlers.adapters.aldrich import parse_aldrich_calendar_payload  # noqa: E402
...
ALDRICH_SOURCE_URL_PREFIX = "https://thealdrich.org/%"


def clear_aldrich_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(ALDRICH_SOURCE_URL_PREFIX),
                    Source.name.like("aldrich_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(ALDRICH_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse The Aldrich Contemporary Art Museum calendar events. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Aldrich DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_aldrich_entries()
        print(
            "Deleted Aldrich rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_aldrich_entries()
        print(
            "Deleted Aldrich rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    run_specs = [
        TargetRunSpec(
            name="aldrich_calendar",
            source_url=ALDRICH_CALENDAR_URL,
            load_payload=load_aldrich_calendar_payload,
            parse_payload=parse_aldrich_calendar_payload,
            parser_name="run_aldrich_parser",
            adapter_type="aldrich_calendar",
            parsed_label="Aldrich rows",
