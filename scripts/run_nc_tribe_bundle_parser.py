...
from src.crawlers.adapters.nc_tribe_bundle import NC_TRIBE_VENUES  # noqa: E402
from src.crawlers.adapters.nc_tribe_bundle import NC_TRIBE_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.nc_tribe_bundle import get_nc_tribe_source_prefixes  # noqa: E402
from src.crawlers.adapters.nc_tribe_bundle import load_nc_tribe_bundle_payload  # noqa: E402
from src.crawlers.adapters.nc_tribe_bundle import parse_nc_tribe_events  # noqa: E402
...
NC_TRIBE_SOURCE_URL_PREFIXES = get_nc_tribe_source_prefixes()
...
def _source_url_prefixes_for_venues(venues) -> tuple[str, ...]:
    ...
    return tuple(dict.fromkeys(prefixes)) or NC_TRIBE_SOURCE_URL_PREFIXES


def clear_nc_tribe_entries(venues=None) -> dict[str, int]:
    deleted_activity_tags = 0
    deleted_activities = 0
    deleted_ingestion_runs = 0
    deleted_sources = 0
    selected_venues = list(venues) if venues is not None else list(NC_TRIBE_VENUES)
    ...
    with SessionLocal() as db:
        ...
        if len(selected_venues) == len(NC_TRIBE_VENUES):
            source_conditions.append(Source.name.like("nc_tribe_%"))
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse North Carolina museums using Tribe events APIs. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(NC_TRIBE_VENUES_BY_SLUG),
        default="all",
        help="Run all NC Tribe venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all NC Tribe bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    selected_venues = (
        list(NC_TRIBE_VENUES)
        if args.venue == "all"
        else [NC_TRIBE_VENUES_BY_SLUG[args.venue]]
    )

    if args.clear and not args.commit:
        deleted = clear_nc_tribe_entries(selected_venues)
        print(
            "Deleted NC Tribe rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_nc_tribe_entries(selected_venues)
        print(
            "Deleted NC Tribe rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    payload = await load_nc_tribe_bundle_payload(
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[nc-tribe-fetch] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any NC Tribe venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        print(f"[nc-tribe-fetch] completed with fetch failures: {failed}")
        send_crawler_alert(
            title="NC Tribe bundle sub-venue fetch failures",
            message=(
                f"Committed the venues that parsed; skipped {len(errors_by_slug)} that could not be fetched."
            ),
            details={
                "parser": "run_nc_tribe_bundle_parser",
