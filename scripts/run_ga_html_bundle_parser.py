...
from src.crawlers.adapters.ga_html_bundle import GA_HTML_VENUES  # noqa: E402
from src.crawlers.adapters.ga_html_bundle import GA_HTML_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.ga_html_bundle import get_ga_html_source_prefixes  # noqa: E402
from src.crawlers.adapters.ga_html_bundle import load_ga_html_bundle_payload  # noqa: E402
from src.crawlers.adapters.ga_html_bundle import parse_ga_html_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Georgia museums using custom HTML/Playwright parsing. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(GA_HTML_VENUES_BY_SLUG),
        default="all",
        help="Run all GA HTML venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all GA HTML bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    for venue in selected_venues:
        targets.append(
            TargetRunSpec(
                name=f"ga_html_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_ga_html_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_ga_html_bundle_parser",
