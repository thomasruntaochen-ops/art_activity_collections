...
from src.crawlers.adapters.fairfield import FAIRFIELD_CALENDAR_URL  # noqa: E402
from src.crawlers.adapters.fairfield import load_fairfield_calendar_payload  # noqa: E402
from src.crawlers.adapters.fairfield import parse_fairfield_calendar_payload  # noqa: E402
...
FAIRFIELD_SOURCE_URL_PREFIX = "https://events.fairfield.edu/%"


def clear_fairfield_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(FAIRFIELD_SOURCE_URL_PREFIX),
                    Source.name.like("fairfield_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(FAIRFIELD_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Fairfield University Art Museum calendar events. "
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional cap on fetched calendar pages.",
    )
    parser.add_argument(
        "--commit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Fairfield University Art Museum DB rows "
    ...
    if args.clear and not args.commit:
        deleted = clear_fairfield_entries()
        print(
            "Deleted Fairfield University Art Museum rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_fairfield_entries()
        print(
            "Deleted Fairfield University Art Museum rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    run_specs = [
        TargetRunSpec(
            name="fairfield_calendar",
            source_url=FAIRFIELD_CALENDAR_URL,
            load_payload=lambda: load_fairfield_calendar_payload(page_limit=args.page_limit),
            parse_payload=parse_fairfield_calendar_payload,
            parser_name="run_fairfield_parser",
            adapter_type="fairfield_calendar",
            parsed_label="Fairfield rows",
