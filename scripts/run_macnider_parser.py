...
from src.crawlers.adapters.macnider import MACNIDER_VENUE_NAME  # noqa: E402
...
MACNIDER_SOURCE_URL_PREFIX = "https://macniderart.org/%"


def clear_macnider_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [(MACNIDER_VENUE_NAME, "Mason City", "IA")])

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(MACNIDER_SOURCE_URL_PREFIX),
                    Source.name == "macnider_events",
                    Source.adapter_type == "macnider_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(MACNIDER_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    ...
    if args.clear and not args.commit:
        deleted = clear_macnider_entries()
        print(
            "Deleted MacNider rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_macnider_entries()
        print(
            "Deleted MacNider rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
