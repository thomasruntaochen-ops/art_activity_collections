...
from src.crawlers.adapters.everson import EVERSON_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.everson import load_everson_payload  # noqa: E402
from src.crawlers.adapters.everson import parse_everson_payload  # noqa: E402
...
EVERSON_SOURCE_URL_PREFIX = "https://everson.org/%"


def clear_everson_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(EVERSON_SOURCE_URL_PREFIX),
                    Source.name.like("everson_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(EVERSON_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Everson Museum of Art event pages. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Everson DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_everson_entries()
        print(
            "Deleted Everson rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_everson_entries()
        print(
            "Deleted Everson rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="everson",
                source_url=EVERSON_EVENTS_URL,
                load_payload=load_everson_payload,
                parse_payload=parse_everson_payload,
                parser_name="run_everson_parser",
                adapter_type="everson_events",
                parsed_label="Everson rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Everson Museum of Art"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_everson_parser",
            source_url=EVERSON_EVENTS_URL,
            details={"source": "Everson Museum of Art"},
        ),
    )
