...
from src.crawlers.adapters.ar_custom_calendar_bundle import AR_CUSTOM_CALENDAR_VENUES  # noqa: E402
from src.crawlers.adapters.ar_custom_calendar_bundle import AR_CUSTOM_CALENDAR_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.ar_custom_calendar_bundle import get_ar_custom_calendar_source_prefixes  # noqa: E402
from src.crawlers.adapters.ar_custom_calendar_bundle import load_ar_custom_calendar_bundle_payload  # noqa: E402
from src.crawlers.adapters.ar_custom_calendar_bundle import parse_ar_custom_calendar_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Arkansas museums using custom calendar detail pages. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(AR_CUSTOM_CALENDAR_VENUES_BY_SLUG),
        default="all",
        help="Run all AR custom calendar venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all AR custom-calendar bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    selected_venues = (
        list(AR_CUSTOM_CALENDAR_VENUES)
        if args.venue == "all"
        else [AR_CUSTOM_CALENDAR_VENUES_BY_SLUG[args.venue]]
    )
    ...
    for venue in selected_venues:
        targets.append(
            TargetRunSpec(
                name=f"ar_custom_{venue.slug}",
                source_url=venue.list_urls[0],
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_ar_custom_calendar_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_ar_custom_calendar_bundle_parser",
