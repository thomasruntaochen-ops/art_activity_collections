...
from src.crawlers.adapters.suma import SUMA_CALENDAR_URL  # noqa: E402
from src.crawlers.adapters.suma import load_suma_payload  # noqa: E402
from src.crawlers.adapters.suma import parse_suma_payload  # noqa: E402
...
SUMA_SOURCE_URL_PREFIX = "https://events.suu.edu/%"


def clear_suma_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(SUMA_SOURCE_URL_PREFIX),
                    Source.name.like("suma_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(SUMA_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Southern Utah Museum of Art events from the SUU calendar. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all SUMA DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_suma_entries()
        print(
            "Deleted SUMA rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="suma_events",
                source_url=SUMA_CALENDAR_URL,
                load_payload=lambda: asyncio.sleep(0, result=payload),
                parse_payload=parse_suma_payload,
                parser_name="run_suma_parser",
                adapter_type="suma_events",
                parsed_label="SUMA rows",
