...
from src.crawlers.adapters.hi_html_bundle import HI_HTML_VENUES  # noqa: E402
from src.crawlers.adapters.hi_html_bundle import HI_HTML_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.hi_html_bundle import get_hi_html_source_prefixes  # noqa: E402
from src.crawlers.adapters.hi_html_bundle import load_hi_html_bundle_payload  # noqa: E402
from src.crawlers.adapters.hi_html_bundle import parse_hi_html_events  # noqa: E402
...
HI_HTML_SOURCE_URL_PREFIXES = get_hi_html_source_prefixes()


def clear_hi_html_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [(venue.venue_name, venue.city, venue.state) for venue in HI_HTML_VENUES],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.in_([url for venue in HI_HTML_VENUES for url in venue.list_urls]),
                    Source.name.like("hi_%"),
                    Source.name.in_([venue.source_name for venue in HI_HTML_VENUES]),
                )
            )
        ).all()

        url_filters = [Activity.source_url.like(f"{prefix}%") for prefix in HI_HTML_SOURCE_URL_PREFIXES]
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Hawaii museums using HTML/detail-page parsing. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(HI_HTML_VENUES_BY_SLUG),
        default="all",
        help="Run all HI HTML bundle venues or one venue slug.",
    )
    parser.add_argument(
        "--commit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all HI HTML bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    selected_venues = (
        list(HI_HTML_VENUES)
        if args.venue == "all"
        else [HI_HTML_VENUES_BY_SLUG[args.venue]]
    )

    if args.clear and not args.commit:
        deleted = clear_hi_html_entries()
        print(
            "Deleted HI HTML rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_hi_html_entries()
        print(
            "Deleted HI HTML rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    for venue in selected_venues:
        targets.append(
            TargetRunSpec(
                name=f"hi_html_{venue.slug}",
                source_url=venue.list_urls[0],
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=(payload.get("payload_by_slug") or {}).get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_hi_html_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_hi_html_bundle_parser",
