...
from src.crawlers.adapters.massmoca import MASSMOCA_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.massmoca import load_massmoca_payload  # noqa: E402
from src.crawlers.adapters.massmoca import parse_massmoca_payload  # noqa: E402
...
MASSMOCA_SOURCE_URL_PREFIX = "https://massmoca.org/%"


def clear_massmoca_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(MASSMOCA_SOURCE_URL_PREFIX),
                    Source.name.like("massmoca_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(MASSMOCA_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse MASS MoCA events API. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all MASS MoCA DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_massmoca_entries()
        print(
            "Deleted MASS MoCA rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_massmoca_entries()
        print(
            "Deleted MASS MoCA rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="massmoca",
                source_url=MASSMOCA_EVENTS_URL,
                load_payload=lambda: load_massmoca_payload(
                    page_limit=args.page_limit,
                    start_date=args.start_date,
                    end_date=args.end_date,
                ),
                parse_payload=parse_massmoca_payload,
                parser_name="run_massmoca_parser",
                adapter_type="massmoca_events",
                parsed_label="MASS MoCA rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "MASS MoCA"},
            )
        ],
        commit=args.commit,
