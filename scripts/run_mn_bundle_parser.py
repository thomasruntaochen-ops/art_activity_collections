...
from src.crawlers.adapters.mn_bundle import MN_VENUES  # noqa: E402
from src.crawlers.adapters.mn_bundle import MN_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.mn_bundle import get_mn_source_prefixes  # noqa: E402
from src.crawlers.adapters.mn_bundle import load_mn_bundle_payload  # noqa: E402
from src.crawlers.adapters.mn_bundle import parse_mn_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Minnesota museum activities from a mixed HTML/API bundle. "
            "Print parsed rows, and optionally commit to MySQL."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(MN_VENUES_BY_SLUG),
        default="all",
        help="Run all Minnesota bundle venues or one venue slug.",
    )
    parser.add_argument(
        "--commit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Minnesota bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    for venue in selected_venues:
        ...
        targets.append(
            TargetRunSpec(
                name=f"mn_bundle_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload_by_slug.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_mn_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_mn_bundle_parser",
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[mn-bundle] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any MN bundle venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed MN bundle with fetch failures: {failed}")
