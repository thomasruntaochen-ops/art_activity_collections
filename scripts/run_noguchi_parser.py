...
from src.crawlers.adapters.noguchi import NOGUCHI_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.noguchi import load_noguchi_payload  # noqa: E402
from src.crawlers.adapters.noguchi import parse_noguchi_payload  # noqa: E402
...
NOGUCHI_SOURCE_URL_PREFIX = "https://www.noguchi.org/%"


def clear_noguchi_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(NOGUCHI_SOURCE_URL_PREFIX),
                    Source.name.like("noguchi_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(NOGUCHI_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse The Noguchi Museum calendar page. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument("--url", default=NOGUCHI_EVENTS_URL)
    parser.add_argument(
        "--commit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Noguchi DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_noguchi_entries()
        print(
            "Deleted Noguchi rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_noguchi_entries()
        print(
            "Deleted Noguchi rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="noguchi",
                source_url=args.url,
                load_payload=lambda: load_noguchi_payload(args.url),
                parse_payload=lambda payload: parse_noguchi_payload(payload, list_url=args.url),
                parser_name="run_noguchi_parser",
                adapter_type="noguchi_events",
                parsed_label="Noguchi rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "The Noguchi Museum"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_noguchi_parser",
            source_url=args.url,
            details={"source": "The Noguchi Museum"},
        ),
    )
