...
from src.crawlers.adapters.nv_bundle import NV_VENUES  # noqa: E402
from src.crawlers.adapters.nv_bundle import NV_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.nv_bundle import get_nv_source_prefixes  # noqa: E402
from src.crawlers.adapters.nv_bundle import load_nv_bundle_payload  # noqa: E402
from src.crawlers.adapters.nv_bundle import parse_nv_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Nevada museum activities from a mixed HTML bundle. "
            "Print parsed rows, and optionally commit to MySQL."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(NV_VENUES_BY_SLUG),
        default="all",
        help="Run all NV bundle venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all NV bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    for venue in selected_venues:
        targets.append(
            TargetRunSpec(
                name=f"nv_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_nv_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_nv_bundle_parser",
