...
from src.crawlers.adapters.fort_smith_regional_art_museum import FSRAM_CITY  # noqa: E402
from src.crawlers.adapters.fort_smith_regional_art_museum import FSRAM_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.fort_smith_regional_art_museum import FSRAM_STATE  # noqa: E402
from src.crawlers.adapters.fort_smith_regional_art_museum import FSRAM_VENUE_NAME  # noqa: E402
from src.crawlers.adapters.fort_smith_regional_art_museum import fetch_fsram_events_page  # noqa: E402
from src.crawlers.adapters.fort_smith_regional_art_museum import parse_fsram_events_html  # noqa: E402
...
def clear_fsram_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [(FSRAM_VENUE_NAME, FSRAM_CITY, FSRAM_STATE)],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url == FSRAM_EVENTS_URL,
                    Source.name == "fort_smith_regional_art_museum_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like("https://fsram.org/%")
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Fort Smith Regional Art Museum education activities. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Fort Smith Regional Art Museum DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_fsram_entries()
        print(
            "Deleted Fort Smith RAM rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_fsram_entries()
        print(
            "Deleted Fort Smith RAM rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    html = await fetch_fsram_events_page(FSRAM_EVENTS_URL)
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="fort_smith_regional_art_museum",
                source_url=FSRAM_EVENTS_URL,
                load_payload=lambda: asyncio.sleep(0, result=html),
                parse_payload=lambda payload: parse_fsram_events_html(payload, list_url=FSRAM_EVENTS_URL),
                parser_name="run_fort_smith_regional_art_museum_parser",
                adapter_type="fort_smith_regional_art_museum_events",
                parsed_label="fsram rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": FSRAM_VENUE_NAME},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_fort_smith_regional_art_museum_parser",
            source_url=FSRAM_EVENTS_URL,
            details={"venue": FSRAM_VENUE_NAME},
        ),
    )
