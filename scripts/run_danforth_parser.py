...
from src.crawlers.adapters.danforth import DANFORTH_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.danforth import load_danforth_payload  # noqa: E402
from src.crawlers.adapters.danforth import parse_danforth_payload  # noqa: E402
...
DANFORTH_SOURCE_URL_PREFIX = "https://danforth.framingham.edu/%"


def clear_danforth_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [("Danforth Art Museum and Art School", "Framingham", "MA")],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(DANFORTH_SOURCE_URL_PREFIX),
                    Source.name.like("danforth_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(DANFORTH_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Danforth Art Museum and Art School events. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Danforth DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_danforth_entries()
        print(
            "Deleted Danforth rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_danforth_entries()
        print(
            "Deleted Danforth rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="danforth",
                source_url=DANFORTH_EVENTS_URL,
                load_payload=load_danforth_payload,
                parse_payload=parse_danforth_payload,
                parser_name="run_danforth_parser",
                adapter_type="danforth_events",
                parsed_label="Danforth rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Danforth Art Museum and Art School"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_danforth_parser",
            source_url=DANFORTH_EVENTS_URL,
            details={"source": "Danforth Art Museum and Art School"},
        ),
    )
