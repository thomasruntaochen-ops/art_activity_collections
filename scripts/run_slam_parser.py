...
from src.crawlers.adapters.slam import SLAM_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.slam import load_slam_payload  # noqa: E402
from src.crawlers.adapters.slam import parse_slam_payload  # noqa: E402
...
SLAM_SOURCE_BASE = "https://www.slam.org"


def clear_slam_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url == SLAM_SOURCE_BASE,
                    Source.base_url.like(f"{SLAM_SOURCE_BASE}/%"),
                    Source.name.like("slam_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(f"{SLAM_SOURCE_BASE}/%")
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Saint Louis Art Museum events from the Tribe events API. "
    ...
    payload = await load_slam_payload(page_limit=args.page_limit)
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_slam_entries()
        print(
            "Deleted SLAM rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="slam_events",
                source_url=SLAM_EVENTS_URL,
                load_payload=lambda: asyncio.sleep(0, result=payload),
                parse_payload=parse_slam_payload,
                parser_name="run_slam_parser",
                adapter_type="slam_events",
                parsed_label="SLAM rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Saint Louis Art Museum"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_slam_parser",
            source_url=SLAM_EVENTS_URL,
            details={"source": "Saint Louis Art Museum"},
        ),
    )
