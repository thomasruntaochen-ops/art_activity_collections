...
from src.crawlers.adapters.moca_la import MOCA_PROGRAMS_URL  # noqa: E402
from src.crawlers.adapters.moca_la import fetch_moca_programs_page  # noqa: E402
from src.crawlers.adapters.moca_la import parse_moca_programs_html  # noqa: E402
...
DEFAULT_CACHE_DIR = Path("data") / "html" / "moca_la"
MOCA_SOURCE_URL_PREFIX = "https://www.moca.org/%"


def _write_html_cache(html: str, cache_dir: Path) -> Path:
    cache_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    output_path = cache_dir / f"moca_programs_{stamp}.html"
...
def _write_text_dump(
    ...
    *,
    ...
    if source_html_path is not None:
        ...
        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        output_path = dump_dir / f"moca_programs_{stamp}.txt"
    ...
    *,
    ...
    if input_html:
        ...
        if not input_path.exists():
            ...
            raise SystemExit(1)
        print(f"Loading MOCA HTML from file: {input_path}")
        html = input_path.read_text(encoding="utf-8")
        return html, input_path

    try:
        html = await fetch_moca_programs_page(url)
    ...
    if save_html:
        ...
        print(f"Saved raw MOCA HTML cache to: {cache_path}")

    return html, None


def clear_moca_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(MOCA_SOURCE_URL_PREFIX),
                    Source.name.like("moca_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(MOCA_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse MOCA programs page filtered to category 25. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument("--url", default=MOCA_PROGRAMS_URL)
    ...
    parser.add_argument(
        ...
        help="Directory used by --save-html (default: data/html/moca_la).",
    )
    parser.add_argument(
        "--dump-text",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all MOCA DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.dump_text:
        ...
        print(f"Saved MOCA text dump to: {dump_path}")
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_moca_entries()
        print(
            "Deleted MOCA rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    def _parse_html(payload: str):
        parsed = parse_moca_programs_html(html=payload, list_url=args.url)
