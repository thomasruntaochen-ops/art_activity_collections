...
from src.crawlers.adapters.mi_bundle import MI_VENUES  # noqa: E402
from src.crawlers.adapters.mi_bundle import MI_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.mi_bundle import get_mi_source_prefixes  # noqa: E402
from src.crawlers.adapters.mi_bundle import load_mi_bundle_payload  # noqa: E402
from src.crawlers.adapters.mi_bundle import parse_mi_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Michigan museum activities from a mixed HTML/API bundle. "
            "Print parsed rows, and optionally commit to MySQL."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(MI_VENUES_BY_SLUG),
        default="all",
        help="Run all MI venues or one venue slug.",
    )
    parser.add_argument(
        "--commit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all MI bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    for venue in selected_venues:
        targets.append(
            TargetRunSpec(
                name=f"mi_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_mi_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_mi_bundle_parser",
