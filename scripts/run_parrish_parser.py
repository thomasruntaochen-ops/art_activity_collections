...
from src.crawlers.adapters.parrish import PARRISH_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.parrish import load_parrish_payload  # noqa: E402
from src.crawlers.adapters.parrish import parse_parrish_payload  # noqa: E402
...
PARRISH_SOURCE_URL_PREFIX = "https://parrishart.org/%"


def clear_parrish_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(PARRISH_SOURCE_URL_PREFIX),
                    Source.name.like("parrish_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(PARRISH_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Parrish Art Museum events API. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Parrish DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_parrish_entries()
        print(
            "Deleted Parrish rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_parrish_entries()
        print(
            "Deleted Parrish rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="parrish",
                source_url=PARRISH_EVENTS_URL,
                load_payload=lambda: load_parrish_payload(page_limit=args.page_limit),
                parse_payload=parse_parrish_payload,
                parser_name="run_parrish_parser",
                adapter_type="parrish_events",
                parsed_label="Parrish rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Parrish Art Museum"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_parrish_parser",
            source_url=PARRISH_EVENTS_URL,
            details={"source": "Parrish Art Museum"},
        ),
    )
