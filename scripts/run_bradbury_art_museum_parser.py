...
from src.crawlers.adapters.bradbury_art_museum import BRADBURY_CITY  # noqa: E402
from src.crawlers.adapters.bradbury_art_museum import BRADBURY_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.bradbury_art_museum import BRADBURY_STATE  # noqa: E402
from src.crawlers.adapters.bradbury_art_museum import BRADBURY_VENUE_NAME  # noqa: E402
from src.crawlers.adapters.bradbury_art_museum import fetch_bradbury_events_page  # noqa: E402
from src.crawlers.adapters.bradbury_art_museum import parse_bradbury_events_html  # noqa: E402
...
def clear_bradbury_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [(BRADBURY_VENUE_NAME, BRADBURY_CITY, BRADBURY_STATE)],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url == BRADBURY_EVENTS_URL,
                    Source.name == "bradbury_art_museum_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like("https://bradburyartmuseum.org/%")
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fetch and parse Bradbury Art Museum events. Print parsed rows, and optionally commit to DB."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Bradbury Art Museum DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_bradbury_entries()
        print(
            "Deleted Bradbury rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_bradbury_entries()
        print(
            "Deleted Bradbury rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    html = await fetch_bradbury_events_page(BRADBURY_EVENTS_URL)
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="bradbury_art_museum",
                source_url=BRADBURY_EVENTS_URL,
                load_payload=lambda: asyncio.sleep(0, result=html),
                parse_payload=lambda payload: parse_bradbury_events_html(payload, list_url=BRADBURY_EVENTS_URL),
                parser_name="run_bradbury_art_museum_parser",
                adapter_type="bradbury_art_museum_events",
                parsed_label="bradbury rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": BRADBURY_VENUE_NAME},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_bradbury_art_museum_parser",
            source_url=BRADBURY_EVENTS_URL,
            details={"venue": BRADBURY_VENUE_NAME},
        ),
    )
