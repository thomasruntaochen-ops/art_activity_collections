...
from src.crawlers.adapters.fl_tribe_bundle import FL_TRIBE_VENUES  # noqa: E402
from src.crawlers.adapters.fl_tribe_bundle import FL_TRIBE_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.fl_tribe_bundle import get_fl_tribe_source_prefixes  # noqa: E402
from src.crawlers.adapters.fl_tribe_bundle import load_fl_tribe_bundle_payload  # noqa: E402
from src.crawlers.adapters.fl_tribe_bundle import parse_fl_tribe_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Florida museums using Tribe events APIs. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(FL_TRIBE_VENUES_BY_SLUG),
        default="all",
        help="Run all FL Tribe venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all FL Tribe bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    selected_venues = (
        list(FL_TRIBE_VENUES)
        if args.venue == "all"
        else [FL_TRIBE_VENUES_BY_SLUG[args.venue]]
    )
    ...
    payload = await load_fl_tribe_bundle_payload(
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[fl-tribe-fetch] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any FL Tribe venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed FL Tribe bundle with fetch failures: {failed}")
