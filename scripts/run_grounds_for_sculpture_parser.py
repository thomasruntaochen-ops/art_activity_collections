...
from src.crawlers.adapters.grounds_for_sculpture import GROUNDS_FOR_SCULPTURE_CALENDAR_URL  # noqa: E402
from src.crawlers.adapters.grounds_for_sculpture import load_grounds_for_sculpture_calendar_payload  # noqa: E402
from src.crawlers.adapters.grounds_for_sculpture import parse_grounds_for_sculpture_calendar_payload  # noqa: E402
...
GROUNDS_FOR_SCULPTURE_SOURCE_URL_PREFIX = "https://www.groundsforsculpture.org/%"


def clear_grounds_for_sculpture_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(GROUNDS_FOR_SCULPTURE_SOURCE_URL_PREFIX),
                    Source.name.like("grounds_for_sculpture_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(GROUNDS_FOR_SCULPTURE_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Grounds For Sculpture calendar events. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Grounds For Sculpture DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_grounds_for_sculpture_entries()
        print(
            "Deleted Grounds For Sculpture rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_grounds_for_sculpture_entries()
        print(
            "Deleted Grounds For Sculpture rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    run_specs = [
        TargetRunSpec(
            name="grounds_for_sculpture_calendar",
            source_url=GROUNDS_FOR_SCULPTURE_CALENDAR_URL,
            load_payload=load_grounds_for_sculpture_calendar_payload,
            parse_payload=parse_grounds_for_sculpture_calendar_payload,
            parser_name="run_grounds_for_sculpture_parser",
            adapter_type="grounds_for_sculpture_calendar",
            parsed_label="Grounds For Sculpture rows",
    ...
    if args.commit:
        print(
            "Committed Grounds For Sculpture rows: "
