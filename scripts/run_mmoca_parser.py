...
from src.crawlers.adapters.mmoca import MMOCA_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.mmoca import load_mmoca_payload  # noqa: E402
from src.crawlers.adapters.mmoca import parse_mmoca_payload  # noqa: E402
...
MMOCA_VENUE = ("Madison Museum of Contemporary Art", "Madison", "WI")
MMOCA_SOURCE_URL_PREFIX = "https://www.mmoca.org/events/"


def clear_mmoca_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [MMOCA_VENUE])
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url == MMOCA_EVENTS_URL,
                    Source.name == "mmoca_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(f"{MMOCA_SOURCE_URL_PREFIX}%")
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Madison Museum of Contemporary Art event pages. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete MMoCA DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_mmoca_entries()
        print(
            "Deleted MMoCA rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="mmoca_events",
                source_url=MMOCA_EVENTS_URL,
                load_payload=lambda: asyncio.sleep(0, result=payload),
                parse_payload=parse_mmoca_payload,
                parser_name="run_mmoca_parser",
                adapter_type="mmoca_events",
                parsed_label="MMoCA rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details=None,
            )
        ],
        commit=args.commit,
