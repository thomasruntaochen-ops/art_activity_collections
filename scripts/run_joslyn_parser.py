...
JOSLYN_SOURCE_URL_PREFIX = "https://joslyn.org/%"


def clear_joslyn_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [("Joslyn Art Museum", "Omaha", "NE")])

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.name == "joslyn_events",
                    Source.adapter_type == "joslyn_events",
                )
            )
        ).all()
        ...
        if venue_ids:
            activity_filter = Activity.venue_id.in_(venue_ids)
        if source_ids:
            source_predicate = Activity.source_id.in_(source_ids)
            activity_filter = source_predicate if activity_filter is None else or_(activity_filter, source_predicate)
...
async def main() -> None:
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_joslyn_entries()
        print(
            "Deleted Joslyn rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
