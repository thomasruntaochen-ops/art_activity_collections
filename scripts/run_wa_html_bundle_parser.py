...
def clear_wa_html_entries(*, venues=None) -> dict[str, int]:
    venues_to_clear = list(venues) if venues is not None else list(WA_HTML_VENUES)
    ...
    with SessionLocal() as db:
        ...
        if len(venues_to_clear) == len(WA_HTML_VENUES):
            source_filters.append(Source.name.like("wa_html_%"))
        source_ids = db.scalars(select(Source.id).where(or_(*source_filters))).all()

        url_filters = [
            Activity.source_url.like(f"{prefix}%")
            for prefix in get_wa_html_source_prefixes(venues_to_clear)
        ]
...
async def main() -> None:
    ...
    if args.clear and not args.commit:
        deleted = clear_wa_html_entries(venues=selected_venues)
        print(
