...
MASSILLON_SOURCE_URL_PREFIXES = (
    "https://www.massillonmuseum.org/",
    "https://collections.humanitix.com/",
    "https://events.humanitix.com/",
)


def clear_massillon_museum_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [("Massillon Museum", "Massillon", "OH")])

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url == MASSILLON_EVENTS_URL,
                    Source.name == "massillon_museum_events",
                )
            )
        ).all()

        url_filters = [Activity.source_url.like(f"{prefix}%") for prefix in MASSILLON_SOURCE_URL_PREFIXES]
...
async def main() -> None:
    ...
    if args.clear and not args.commit:
        deleted = clear_massillon_museum_entries()
        print(
            "Deleted Massillon Museum rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_massillon_museum_entries()
        print(
            "Deleted Massillon Museum rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                ...
                empty_parse_details={"source": "Massillon Museum"},
            )
        ],
        commit=args.commit,
