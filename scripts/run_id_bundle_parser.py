...
from src.crawlers.adapters.id_bundle import ID_VENUES  # noqa: E402
from src.crawlers.adapters.id_bundle import ID_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.id_bundle import get_id_source_prefixes  # noqa: E402
from src.crawlers.adapters.id_bundle import load_id_bundle_payload  # noqa: E402
from src.crawlers.adapters.id_bundle import parse_id_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Idaho museum activities from Boise Art Museum, "
            "Sun Valley Museum of Art, and The Art Museum of Eastern Idaho."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(ID_VENUES_BY_SLUG),
        default="all",
        help="Run all Idaho venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Idaho bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    payload = await load_id_bundle_payload(venues=selected_venues)
    ...
    for venue in selected_venues:
        ...
        targets.append(
            TargetRunSpec(
                name=f"id_bundle_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload_by_slug.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_id_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_id_bundle_parser",
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[id-bundle] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any ID bundle venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed ID bundle with fetch failures: {failed}")
