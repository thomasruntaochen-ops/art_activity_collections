...
from src.crawlers.adapters.ky_bundle import KY_VENUES  # noqa: E402
from src.crawlers.adapters.ky_bundle import KY_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.ky_bundle import get_ky_source_prefixes  # noqa: E402
from src.crawlers.adapters.ky_bundle import load_ky_bundle_payload  # noqa: E402
from src.crawlers.adapters.ky_bundle import parse_ky_events  # noqa: E402
...
KY_SOURCE_URL_PREFIXES = get_ky_source_prefixes()


def clear_ky_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [(venue.venue_name, venue.city, venue.state) for venue in KY_VENUES],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.in_([url for venue in KY_VENUES for url in venue.list_urls]),
                    Source.name.like("ky_%_events"),
                    Source.name.in_([venue.source_name for venue in KY_VENUES]),
                )
            )
        ).all()

        url_filters = [Activity.source_url.like(f"{prefix}%") for prefix in KY_SOURCE_URL_PREFIXES]
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Kentucky museum activities from a mixed HTML/JSON bundle. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(KY_VENUES_BY_SLUG),
        default="all",
        help="Run all KY bundle venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all KY bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
            "If used without --commit, the script exits after deletion."
        ),
    )
    args = parser.parse_args()

    selected_venues = list(KY_VENUES) if args.venue == "all" else [KY_VENUES_BY_SLUG[args.venue]]

    if args.clear and not args.commit:
        deleted = clear_ky_entries()
        print(
            "Deleted KY rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_ky_entries()
        print(
            "Deleted KY rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    for venue in selected_venues:
        targets.append(
            TargetRunSpec(
                name=f"ky_{venue.slug}",
                source_url=venue.list_urls[0],
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_ky_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_ky_bundle_parser",
