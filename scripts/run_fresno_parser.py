...
from src.crawlers.adapters.fresno import (  # noqa: E402
    FRESNO_WORKSHOPS_URL,
    fetch_fresno_workshops_page,
    parse_fresno_workshops_html,
)
...
DEFAULT_CACHE_DIR = Path("data") / "html" / "fresno"
FRESNO_SOURCE_URL_PREFIX = "https://fresnoartmuseum.org/%"
...
def _write_html_cache(html: str, cache_dir: Path) -> Path:
    ...
    output_path = cache_dir / f"fresno_workshops_{stamp}.html"
...
def _write_text_dump(
    ...
    *,
    ...
    if source_html_path is not None:
        ...
        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        output_path = dump_dir / f"fresno_workshops_{stamp}.txt"
    ...
    *,
    ...
    if input_html:
        ...
        if not input_path.exists():
            ...
            raise SystemExit(1)
        print(f"Loading Fresno HTML from file: {input_path}")
        html = input_path.read_text(encoding="utf-8")
        return html, input_path

    try:
        html = await fetch_fresno_workshops_page(url)
    ...
    if save_html:
        ...
        print(f"Saved raw Fresno HTML cache to: {cache_path}")

    return html, None


def clear_fresno_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(FRESNO_SOURCE_URL_PREFIX),
                    Source.name.like("fresno_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(FRESNO_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Fresno Art Museum workshop dates. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument("--url", default=FRESNO_WORKSHOPS_URL)
    ...
    parser.add_argument(
        ...
        help="Directory used by --save-html (default: data/html/fresno).",
    )
    parser.add_argument(
        "--dump-text",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Fresno DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.dump_text:
        ...
        print(f"Saved Fresno text dump to: {dump_path}")

    parsed = parse_fresno_workshops_html(html=html, list_url=args.url)
    ...
    if args.clear:
        deleted = clear_fresno_entries()
        print(
            "Deleted Fresno rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    written, stats = upsert_extracted_activities_with_stats(
        source_url=args.url,
        extracted=parsed,
        adapter_type="fresno_workshops",
    )
    print(
        "DB write summary: "
        f"input={stats.input_rows}, "
        f"deduped={stats.deduped_rows}, "
        f"inserted={stats.inserted}, "
        f"updated={stats.updated}, "
        f"unchanged={stats.unchanged}"
    )
    print(f"Rows considered for write: {len(written)}")
