...
from src.crawlers.adapters.mocact import MOCACT_EVENTS_PAGE_URL  # noqa: E402
from src.crawlers.adapters.mocact import load_mocact_events_payload  # noqa: E402
from src.crawlers.adapters.mocact import parse_mocact_events_payload  # noqa: E402
...
MOCACT_SOURCE_URL_PREFIX = "https://mocact.org/%"


def clear_mocact_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(MOCACT_SOURCE_URL_PREFIX),
                    Source.name.like("mocact_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(MOCACT_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse MoCA CT qualifying events. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all MoCA CT DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_mocact_entries()
        print(
            "Deleted MoCA CT rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_mocact_entries()
        print(
            "Deleted MoCA CT rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    run_specs = [
        TargetRunSpec(
            ...
            adapter_type="mocact_events",
            parsed_label="MoCA CT rows",
