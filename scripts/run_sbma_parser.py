...
async def main() -> None:
    ...
    def _parse_pages(payload: list[tuple[str, str, Path | None]]):
        parsed = parse_sbma_events_pages(
            [(page_url, html) for page_url, html, _ in payload],
            list_url=args.url,
        )
        parsed.sort(key=lambda row: (row.start_at, row.title, row.source_url))
        return parsed

    async def _load_cached_pages() -> list[tuple[str, str, Path | None]]:
        return loaded_pages
