...
KATONAH_SOURCE_URL_PREFIX = "https://www.katonahmuseum.org/%"


def clear_katonah_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(KATONAH_SOURCE_URL_PREFIX),
                    Source.name.like("katonah_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(KATONAH_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Katonah Museum of Art events page. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument("--url", default=KATONAH_EVENTS_URL)
    parser.add_argument(
        "--commit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Katonah DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_katonah_entries()
        print(
            "Deleted Katonah rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_katonah_entries()
        print(
            "Deleted Katonah rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                ...
                empty_parse_details={"source": "Katonah Museum of Art"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_katonah_parser",
            source_url=args.url,
            details={"source": "Katonah Museum of Art"},
        ),
    )
