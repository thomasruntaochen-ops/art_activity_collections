...
from src.crawlers.adapters.dc_tribe_bundle import DC_TRIBE_VENUES  # noqa: E402
from src.crawlers.adapters.dc_tribe_bundle import DC_TRIBE_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.dc_tribe_bundle import get_dc_tribe_source_prefixes  # noqa: E402
from src.crawlers.adapters.dc_tribe_bundle import load_dc_tribe_bundle_payload  # noqa: E402
from src.crawlers.adapters.dc_tribe_bundle import parse_dc_tribe_events  # noqa: E402
...
DC_TRIBE_SOURCE_URL_PREFIXES = get_dc_tribe_source_prefixes()


def clear_dc_tribe_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [(venue.venue_name, venue.city, venue.state) for venue in DC_TRIBE_VENUES],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.in_([venue.list_url for venue in DC_TRIBE_VENUES]),
                    Source.name.like("dc_tribe_%"),
                    Source.name.in_([venue.source_name for venue in DC_TRIBE_VENUES]),
                )
            )
        ).all()

        url_filters = [Activity.source_url.like(f"{prefix}%") for prefix in DC_TRIBE_SOURCE_URL_PREFIXES]
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse DC museums using Tribe events APIs. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(DC_TRIBE_VENUES_BY_SLUG),
        default="all",
        help="Run all DC Tribe venues or one venue slug.",
    )
    parser.add_argument(
        "--commit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all DC Tribe bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    selected_venues = (
        list(DC_TRIBE_VENUES)
        if args.venue == "all"
        else [DC_TRIBE_VENUES_BY_SLUG[args.venue]]
    )

    if args.clear and not args.commit:
        deleted = clear_dc_tribe_entries()
        print(
            "Deleted DC Tribe rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_dc_tribe_entries()
        print(
            "Deleted DC Tribe rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    payload = await load_dc_tribe_bundle_payload(venues=selected_venues)
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[dc-tribe-fetch] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any DC Tribe venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed DC Tribe bundle with fetch failures: {failed}")
