...
from src.crawlers.adapters.va_tribe_bundle import VA_TRIBE_VENUES  # noqa: E402
from src.crawlers.adapters.va_tribe_bundle import VA_TRIBE_VENUES_BY_SLUG  # noqa: E402
...
VA_TRIBE_SOURCE_URL_PREFIXES = get_va_tribe_source_prefixes()


def clear_va_tribe_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [(venue.venue_name, venue.city, venue.state) for venue in VA_TRIBE_VENUES],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.in_([venue.list_url for venue in VA_TRIBE_VENUES]),
                    Source.name.like("va_tribe_%"),
                    Source.name.in_([venue.source_name for venue in VA_TRIBE_VENUES]),
                )
            )
        ).all()

        url_filters = [Activity.source_url.like(f"{prefix}%") for prefix in VA_TRIBE_SOURCE_URL_PREFIXES]
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Virginia museums using Tribe events APIs. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(VA_TRIBE_VENUES_BY_SLUG),
        default="all",
        help="Run all VA Tribe venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all VA Tribe bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    selected_venues = (
        list(VA_TRIBE_VENUES)
        if args.venue == "all"
        else [VA_TRIBE_VENUES_BY_SLUG[args.venue]]
    )

    if args.clear and not args.commit:
        deleted = clear_va_tribe_entries()
        print(
            "Deleted VA Tribe rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_va_tribe_entries()
        print(
            "Deleted VA Tribe rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
