...
def clear_va_tribe_entries(*, venues: list | None = None) -> dict[str, int]:
    venues_to_clear = list(venues) if venues is not None else list(VA_TRIBE_VENUES)
    ...
    with SessionLocal() as db:
        ...
        if len(venues_to_clear) == len(VA_TRIBE_VENUES):
            source_filters.append(Source.name.like("va_tribe_%"))

        source_ids = db.scalars(select(Source.id).where(or_(*source_filters))).all()

        url_prefixes = get_va_tribe_source_prefixes(tuple(venues_to_clear))
...
async def main() -> None:
    ...
    if args.clear and not args.commit:
        deleted = clear_va_tribe_entries(venues=selected_venues)
        print(
    ...
    def _clear_before_commit() -> None:
        ...
        if clear_completed or not args.clear:
            return
        deleted = clear_va_tribe_entries(venues=venues_to_clear)
        print(
