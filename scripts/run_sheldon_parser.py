...
from src.crawlers.adapters.sheldon import SHELDON_VENUE_NAME  # noqa: E402
...
SHELDON_SOURCE_URL_PREFIX = "https://sheldonartmuseum.org/%"


def clear_sheldon_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [(SHELDON_VENUE_NAME, "Lincoln", "NE")])

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(SHELDON_SOURCE_URL_PREFIX),
                    Source.name == "sheldon_events",
                    Source.adapter_type == "sheldon_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(SHELDON_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    ...
    if args.clear and not args.commit:
        deleted = clear_sheldon_entries()
        print(
            "Deleted Sheldon rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_sheldon_entries()
        print(
            "Deleted Sheldon rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
