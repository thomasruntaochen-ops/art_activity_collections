...
from src.crawlers.adapters.mag import MAG_PROGRAMS_URL  # noqa: E402
from src.crawlers.adapters.mag import load_mag_payload  # noqa: E402
from src.crawlers.adapters.mag import parse_mag_payload  # noqa: E402
...
MAG_SOURCE_URL_PREFIX = "https://mag.rochester.edu/%"


def clear_mag_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(MAG_SOURCE_URL_PREFIX),
                    Source.name.like("mag_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(MAG_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Memorial Art Gallery event API. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all MAG DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_mag_entries()
        print(
            "Deleted MAG rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="mag",
                source_url=MAG_PROGRAMS_URL,
                load_payload=lambda: load_mag_payload(page_limit=args.page_limit),
                parse_payload=parse_mag_payload,
                parser_name="run_mag_parser",
                adapter_type="mag_events",
                parsed_label="MAG rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Memorial Art Gallery"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_mag_parser",
            source_url=MAG_PROGRAMS_URL,
            details={"source": "Memorial Art Gallery"},
        ),
    )
