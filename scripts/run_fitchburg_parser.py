...
from src.crawlers.adapters.fitchburg import FITCHBURG_PROGRAMS_URL  # noqa: E402
from src.crawlers.adapters.fitchburg import load_fitchburg_payload  # noqa: E402
from src.crawlers.adapters.fitchburg import parse_fitchburg_payload  # noqa: E402
...
FITCHBURG_SOURCE_URL_PREFIX = "https://fitchburgartmuseum.org/%"


def clear_fitchburg_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(FITCHBURG_SOURCE_URL_PREFIX),
                    Source.name.like("fitchburg_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(FITCHBURG_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Fitchburg Art Museum upcoming programs. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Fitchburg DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_fitchburg_entries()
        print(
            "Deleted Fitchburg rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_fitchburg_entries()
        print(
            "Deleted Fitchburg rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="fitchburg",
                source_url=FITCHBURG_PROGRAMS_URL,
                load_payload=load_fitchburg_payload,
                parse_payload=parse_fitchburg_payload,
                parser_name="run_fitchburg_parser",
                adapter_type="fitchburg_programs",
                parsed_label="Fitchburg rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Fitchburg Art Museum"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_fitchburg_parser",
            source_url=FITCHBURG_PROGRAMS_URL,
            details={"source": "Fitchburg Art Museum"},
        ),
    )
