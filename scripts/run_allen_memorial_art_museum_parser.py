...
ALLEN_SOURCE_URL_PREFIX = "https://amam.oberlin.edu/%"


def clear_allen_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [("Allen Memorial Art Museum", "Oberlin", "OH")])

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(ALLEN_SOURCE_URL_PREFIX),
                    Source.name == "allen_memorial_art_museum_events",
                    Source.adapter_type == "allen_memorial_art_museum_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(ALLEN_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    ...
    if args.clear and not args.commit:
        deleted = clear_allen_entries()
        print(
            "Deleted Allen Memorial rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_allen_entries()
        print(
            "Deleted Allen Memorial rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
