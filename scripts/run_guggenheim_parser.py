...
from src.crawlers.adapters.guggenheim import GUGGENHEIM_CALENDAR_URL  # noqa: E402
from src.crawlers.adapters.guggenheim import load_guggenheim_payload  # noqa: E402
from src.crawlers.adapters.guggenheim import parse_guggenheim_payload  # noqa: E402
...
GUGGENHEIM_SOURCE_URL_PREFIX = "https://www.guggenheim.org/%"


def clear_guggenheim_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(GUGGENHEIM_SOURCE_URL_PREFIX),
                    Source.name.like("guggenheim_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(GUGGENHEIM_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse the Solomon R. Guggenheim Museum calendar. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Guggenheim DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_guggenheim_entries()
        print(
            "Deleted Solomon R. Guggenheim Museum rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_guggenheim_entries()
        print(
            "Deleted Solomon R. Guggenheim Museum rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="guggenheim",
                source_url=GUGGENHEIM_CALENDAR_URL,
                load_payload=load_guggenheim_payload,
                parse_payload=parse_guggenheim_payload,
                parser_name="run_guggenheim_parser",
                adapter_type="guggenheim_calendar",
                parsed_label="Solomon R. Guggenheim Museum rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Solomon R. Guggenheim Museum"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_guggenheim_parser",
            source_url=GUGGENHEIM_CALENDAR_URL,
            details={"source": "Solomon R. Guggenheim Museum"},
        ),
    )
