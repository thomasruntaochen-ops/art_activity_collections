...
from src.crawlers.adapters.pa_tribe_bundle import PA_TRIBE_VENUES  # noqa: E402
from src.crawlers.adapters.pa_tribe_bundle import PA_TRIBE_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.pa_tribe_bundle import get_pa_tribe_source_prefixes  # noqa: E402
from src.crawlers.adapters.pa_tribe_bundle import load_pa_tribe_bundle_payload  # noqa: E402
from src.crawlers.adapters.pa_tribe_bundle import parse_pa_tribe_events  # noqa: E402
...
PA_TRIBE_SOURCE_URL_PREFIXES = get_pa_tribe_source_prefixes()


def clear_pa_tribe_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [(venue.venue_name, venue.city, venue.state) for venue in PA_TRIBE_VENUES],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.in_([venue.list_url for venue in PA_TRIBE_VENUES]),
                    Source.name.like("pa_tribe_%"),
                    Source.name.in_([venue.source_name for venue in PA_TRIBE_VENUES]),
                )
            )
        ).all()

        url_filters = [Activity.source_url.like(f"{prefix}%") for prefix in PA_TRIBE_SOURCE_URL_PREFIXES]
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Pennsylvania museums using Tribe events APIs. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(PA_TRIBE_VENUES_BY_SLUG),
        default="all",
        help="Run all PA Tribe venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all PA Tribe bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    selected_venues = (
        list(PA_TRIBE_VENUES)
        if args.venue == "all"
        else [PA_TRIBE_VENUES_BY_SLUG[args.venue]]
    )

    if args.clear and not args.commit:
        deleted = clear_pa_tribe_entries()
        print(
            "Deleted PA Tribe rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_pa_tribe_entries()
        print(
            "Deleted PA Tribe rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    payload = await load_pa_tribe_bundle_payload(
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[pa-tribe-fetch] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any PA Tribe venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed PA Tribe bundle with fetch failures: {failed}")
