...
from src.crawlers.adapters.princeton import PRINCETON_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.princeton import load_princeton_events_payload  # noqa: E402
from src.crawlers.adapters.princeton import parse_princeton_events_payload  # noqa: E402
...
PRINCETON_SOURCE_URL_PREFIX = "https://artmuseum.princeton.edu/%"


def clear_princeton_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(PRINCETON_SOURCE_URL_PREFIX),
                    Source.name.like("princeton_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(PRINCETON_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Princeton University Art Museum events. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Princeton DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_princeton_entries()
        print(
            "Deleted Princeton rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_princeton_entries()
        print(
            "Deleted Princeton rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    run_specs = [
        TargetRunSpec(
            name="princeton_events",
            source_url=PRINCETON_EVENTS_URL,
            load_payload=load_princeton_events_payload,
            parse_payload=parse_princeton_events_payload,
            parser_name="run_princeton_parser",
            adapter_type="princeton_events",
            parsed_label="Princeton rows",
