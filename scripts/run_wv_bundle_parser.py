...
from src.crawlers.adapters.wv_bundle import WV_VENUES  # noqa: E402
from src.crawlers.adapters.wv_bundle import WV_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.wv_bundle import get_wv_source_prefixes  # noqa: E402
from src.crawlers.adapters.wv_bundle import load_wv_bundle_payload  # noqa: E402
from src.crawlers.adapters.wv_bundle import parse_wv_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse West Virginia museum activities from mixed official sources. "
            "Print parsed rows, and optionally commit to MySQL."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(WV_VENUES_BY_SLUG),
        default="all",
        help="Run all West Virginia bundle venues or one venue slug.",
    )
    parser.add_argument(
        "--commit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all WV bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    payload = await load_wv_bundle_payload(venues=selected_venues)
    ...
    for venue in selected_venues:
        ...
        targets.append(
            TargetRunSpec(
                name=f"wv_bundle_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload_by_slug.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_wv_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_wv_bundle_parser",
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[wv-bundle] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any WV bundle venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed WV bundle with fetch failures: {failed}")
