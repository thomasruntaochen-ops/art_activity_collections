...
from src.crawlers.adapters.ga_mec_bundle import GA_MEC_VENUES  # noqa: E402
from src.crawlers.adapters.ga_mec_bundle import GA_MEC_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.ga_mec_bundle import get_ga_mec_source_prefixes  # noqa: E402
from src.crawlers.adapters.ga_mec_bundle import load_ga_mec_bundle_payload  # noqa: E402
from src.crawlers.adapters.ga_mec_bundle import parse_ga_mec_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Georgia museums using MEC/detail-page parsing. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(GA_MEC_VENUES_BY_SLUG),
        default="all",
        help="Run all GA MEC venues or one venue slug.",
    )
    ...
    payload = await load_ga_mec_bundle_payload(
    ...
    for venue in selected_venues:
        targets.append(
            TargetRunSpec(
                name=f"ga_mec_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_ga_mec_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_ga_mec_bundle_parser",
