...
from src.crawlers.adapters.new_bedford import NEW_BEDFORD_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.new_bedford import load_new_bedford_payload  # noqa: E402
from src.crawlers.adapters.new_bedford import parse_new_bedford_payload  # noqa: E402
...
NEW_BEDFORD_SOURCE_URL_PREFIX = "https://newbedfordart.org/%"


def clear_new_bedford_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(NEW_BEDFORD_SOURCE_URL_PREFIX),
                    Source.name.like("new_bedford_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(NEW_BEDFORD_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse New Bedford Art Museum events and classes. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all New Bedford DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_new_bedford_entries()
        print(
            "Deleted New Bedford rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_new_bedford_entries()
        print(
            "Deleted New Bedford rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="new_bedford",
                source_url=NEW_BEDFORD_EVENTS_URL,
                load_payload=load_new_bedford_payload,
                parse_payload=parse_new_bedford_payload,
                parser_name="run_new_bedford_parser",
                adapter_type="new_bedford_events",
                parsed_label="New Bedford rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "New Bedford Art Museum"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_new_bedford_parser",
            source_url=NEW_BEDFORD_EVENTS_URL,
            details={"source": "New Bedford Art Museum"},
        ),
    )
