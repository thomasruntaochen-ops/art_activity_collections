...
from src.crawlers.adapters.florence_griswold import FLORENCE_GRISWOLD_EVENTS_PAGE_URL  # noqa: E402
from src.crawlers.adapters.florence_griswold import load_florence_griswold_events_payload  # noqa: E402
from src.crawlers.adapters.florence_griswold import parse_florence_griswold_events_payload  # noqa: E402
...
FLORENCE_GRISWOLD_SOURCE_URL_PREFIX = "https://flogris.org/%"


def clear_florence_griswold_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(FLORENCE_GRISWOLD_SOURCE_URL_PREFIX),
                    Source.name.like("florence_griswold_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(FLORENCE_GRISWOLD_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Florence Griswold Museum qualifying events. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Florence Griswold Museum DB rows "
    ...
    if args.clear and not args.commit:
        deleted = clear_florence_griswold_entries()
        print(
            "Deleted Florence Griswold rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_florence_griswold_entries()
        print(
            "Deleted Florence Griswold rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    run_specs = [
        TargetRunSpec(
            name="florence_griswold_events",
            source_url=FLORENCE_GRISWOLD_EVENTS_PAGE_URL,
            load_payload=lambda: load_florence_griswold_events_payload(
                start_date=args.start_date,
                page_limit=args.page_limit,
            ),
            parse_payload=parse_florence_griswold_events_payload,
            parser_name="run_florence_griswold_parser",
            adapter_type="florence_griswold_events",
            parsed_label="Florence Griswold rows",
    ...
    if args.commit:
        print(
            "Committed Florence Griswold rows: "
