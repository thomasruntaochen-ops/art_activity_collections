...
from src.crawlers.adapters.me_html_bundle import ME_HTML_VENUES  # noqa: E402
from src.crawlers.adapters.me_html_bundle import ME_HTML_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.me_html_bundle import get_me_source_prefixes  # noqa: E402
from src.crawlers.adapters.me_html_bundle import load_me_html_bundle_payload  # noqa: E402
from src.crawlers.adapters.me_html_bundle import parse_me_html_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Maine museum activities from mixed HTML and API-backed sources. "
            "Print parsed rows, and optionally commit to MySQL."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(ME_HTML_VENUES_BY_SLUG),
        default="all",
        help="Run all Maine bundle venues or one venue slug.",
    )
    parser.add_argument(
        "--commit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Maine bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    selected_venues = (
        list(ME_HTML_VENUES)
        if args.venue == "all"
        else [ME_HTML_VENUES_BY_SLUG[args.venue]]
    )
    ...
    payload = await load_me_html_bundle_payload(venues=selected_venues)
    ...
    for venue in selected_venues:
        ...
        targets.append(
            TargetRunSpec(
                name=f"me_html_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload_by_slug.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_me_html_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_me_html_bundle_parser",
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[me-bundle] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any Maine bundle venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed ME bundle with fetch failures: {failed}")
