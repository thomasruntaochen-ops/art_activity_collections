...
from src.crawlers.adapters.va_html_bundle import VA_HTML_VENUES  # noqa: E402
from src.crawlers.adapters.va_html_bundle import VA_HTML_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.va_html_bundle import get_va_html_source_prefixes  # noqa: E402
from src.crawlers.adapters.va_html_bundle import load_va_html_bundle_payload  # noqa: E402
from src.crawlers.adapters.va_html_bundle import parse_va_html_events  # noqa: E402
...
VA_HTML_SOURCE_URL_PREFIXES = get_va_html_source_prefixes()


def clear_va_html_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.in_([venue.list_url for venue in VA_HTML_VENUES]),
                    Source.name.like("va_html_%"),
                    Source.name.in_([venue.source_name for venue in VA_HTML_VENUES]),
                )
            )
        ).all()

        url_filters = [Activity.source_url.like(f"{prefix}%") for prefix in VA_HTML_SOURCE_URL_PREFIXES]
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Virginia museums using HTML/detail-page parsing. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(VA_HTML_VENUES_BY_SLUG),
        default="all",
        help="Run all VA HTML bundle venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all VA HTML bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    selected_venues = (
        list(VA_HTML_VENUES)
        if args.venue == "all"
        else [VA_HTML_VENUES_BY_SLUG[args.venue]]
    )

    if args.clear and not args.commit:
        deleted = clear_va_html_entries()
        print(
            "Deleted VA HTML rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_va_html_entries()
        print(
            "Deleted VA HTML rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    payload = await load_va_html_bundle_payload(
    ...
    for venue in selected_venues:
        targets.append(
            TargetRunSpec(
                name=f"va_html_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_va_html_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_va_html_bundle_parser",
