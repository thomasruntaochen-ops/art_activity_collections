...
from src.crawlers.adapters.nm_bundle import NM_VENUES  # noqa: E402
from src.crawlers.adapters.nm_bundle import NM_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.nm_bundle import get_nm_source_prefixes  # noqa: E402
from src.crawlers.adapters.nm_bundle import load_nm_bundle_payload  # noqa: E402
from src.crawlers.adapters.nm_bundle import parse_nm_events  # noqa: E402
...
def clear_nm_entries(*, venues=None) -> dict[str, int]:
    deleted_activity_tags = 0
    deleted_activities = 0
    deleted_ingestion_runs = 0
    deleted_sources = 0
    selected_venues = list(venues) if venues is not None else list(NM_VENUES)
    ...
    with SessionLocal() as db:
        ...
        source_filters = [Source.name.in_([venue.source_name for venue in selected_venues])]
        if venues is None:
            source_filters.append(Source.name.like("nm_%_events"))
        source_ids = db.scalars(select(Source.id).where(or_(*source_filters))).all()

        url_filters = [Activity.source_url.like(f"{prefix}%") for prefix in get_nm_source_prefixes(selected_venues)]
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse New Mexico museum activities from a mixed HTML/Playwright bundle. "
            "Print parsed rows, and optionally commit to MySQL."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(NM_VENUES_BY_SLUG),
        default="all",
        help="Run all NM venues or one venue slug.",
    )
    parser.add_argument(
        "--commit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all NM bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
            "If used without --commit, the script exits after deletion."
        ),
    )
    args = parser.parse_args()

    selected_venues = list(NM_VENUES) if args.venue == "all" else [NM_VENUES_BY_SLUG[args.venue]]

    if args.clear and not args.commit:
        deleted = clear_nm_entries(venues=selected_venues)
        print(
            "Deleted NM bundle rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_nm_entries(venues=selected_venues)
        print(
            "Deleted NM bundle rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    for venue in selected_venues:
        targets.append(
            TargetRunSpec(
                name=f"nm_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_nm_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_nm_bundle_parser",
