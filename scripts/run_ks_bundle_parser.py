...
from src.crawlers.adapters.ks_bundle import KS_VENUES  # noqa: E402
from src.crawlers.adapters.ks_bundle import KS_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.ks_bundle import get_ks_source_prefixes  # noqa: E402
from src.crawlers.adapters.ks_bundle import load_ks_bundle_payload  # noqa: E402
from src.crawlers.adapters.ks_bundle import parse_ks_events  # noqa: E402
...
KS_SOURCE_URL_PREFIXES = get_ks_source_prefixes()


def clear_ks_bundle_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [(venue.venue_name, venue.city, venue.state) for venue in KS_VENUES],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.in_([venue.list_url for venue in KS_VENUES]),
                    Source.name.like("ks_%_events"),
                    Source.name.in_([venue.source_name for venue in KS_VENUES]),
                )
            )
        ).all()

        url_filters = [Activity.source_url.like(f"{prefix}%") for prefix in KS_SOURCE_URL_PREFIXES]
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Kansas museum activities from venue-specific HTML, XML, JSON, "
            "and widget sources. Print parsed rows, and optionally commit to MySQL."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(KS_VENUES_BY_SLUG),
        default="all",
        help="Run all Kansas venues or one venue slug.",
    )
    parser.add_argument(
        "--start-date",
        default=None,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="How many current/future months to fetch for month-based KS calendars.",
    )
    parser.add_argument(
        "--per-page",
        type=int,
        default=50,
        help="How many rows to request from tribe APIs per page.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all KS bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
            "If used without --commit, the script exits after deletion."
        ),
    )
    args = parser.parse_args()

    selected_venues = list(KS_VENUES) if args.venue == "all" else [KS_VENUES_BY_SLUG[args.venue]]

    if args.clear and not args.commit:
        deleted = clear_ks_bundle_entries()
        print(
            "Deleted KS bundle rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_ks_bundle_entries()
        print(
            "Deleted KS bundle rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    for venue in selected_venues:
        ...
        targets.append(
            TargetRunSpec(
                name=f"ks_bundle_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload_by_slug.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_ks_events(
                    venue_payload,
                    venue=selected_venue,
                    start_date=args.start_date,
                ),
                parser_name="run_ks_bundle_parser",
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[ks-bundle] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any KS bundle venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed KS bundle with fetch failures: {failed}")
