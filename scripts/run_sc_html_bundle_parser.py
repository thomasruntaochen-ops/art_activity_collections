...
from src.crawlers.adapters.sc_html_bundle import SC_HTML_VENUES  # noqa: E402
from src.crawlers.adapters.sc_html_bundle import SC_HTML_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.sc_html_bundle import get_sc_source_prefixes  # noqa: E402
from src.crawlers.adapters.sc_html_bundle import load_sc_html_bundle_payload  # noqa: E402
from src.crawlers.adapters.sc_html_bundle import parse_sc_html_events  # noqa: E402
...
SC_SOURCE_URL_PREFIXES = get_sc_source_prefixes()


def clear_sc_html_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [(venue.venue_name, venue.city, venue.state) for venue in SC_HTML_VENUES],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.in_([venue.list_url for venue in SC_HTML_VENUES]),
                    Source.name.like("sc_%_events"),
                    Source.name.in_([venue.source_name for venue in SC_HTML_VENUES]),
                )
            )
        ).all()

        url_filters = [Activity.source_url.like(f"{prefix}%") for prefix in SC_SOURCE_URL_PREFIXES]
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse South Carolina museum activities from a mixed HTML/detail bundle. "
            "Print parsed rows, and optionally commit to MySQL."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(SC_HTML_VENUES_BY_SLUG),
        default="all",
        help="Run all SC bundle venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all SC bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
            "If used without --commit, the script exits after deletion."
        ),
    )
    args = parser.parse_args()

    selected_venues = list(SC_HTML_VENUES) if args.venue == "all" else [SC_HTML_VENUES_BY_SLUG[args.venue]]

    if args.clear and not args.commit:
        deleted = clear_sc_html_entries()
        print(
            "Deleted SC bundle rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_sc_html_entries()
        print(
            "Deleted SC bundle rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    for venue in selected_venues:
        ...
        targets.append(
            TargetRunSpec(
                name=f"sc_html_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload_by_slug.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_sc_html_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_sc_html_bundle_parser",
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[sc-bundle] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any SC bundle venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed SC bundle with fetch failures: {failed}")
