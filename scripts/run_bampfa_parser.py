...
from src.crawlers.adapters.bampfa import (  # noqa: E402
    BAMPFA_CALENDAR_URL,
    fetch_bampfa_calendar_page,
    parse_bampfa_events_html,
)
...
DEFAULT_CACHE_DIR = Path("data") / "html" / "bampfa"
BAMPFA_SOURCE_URL_PREFIX = "https://bampfa.org/%"

def _write_html_cache(html: str, cache_dir: Path) -> Path:
    cache_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    output_path = cache_dir / f"bampfa_calendar_{stamp}.html"
...
def _write_text_dump(
    ...
    *,
    ...
    if source_html_path is not None:
        ...
        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        output_path = dump_dir / f"bampfa_calendar_{stamp}.txt"
    ...
    *,
    ...
    if input_html:
        ...
        if not input_path.exists():
            ...
            raise SystemExit(1)
        print(f"Loading BAMPFA HTML from file: {input_path}")
        html = input_path.read_text(encoding="utf-8")
        return html, input_path

    try:
        html = await fetch_bampfa_calendar_page(url)
    ...
    if save_html:
        ...
        print(f"Saved raw BAMPFA HTML cache to: {cache_path}")

    return html, None


def clear_bampfa_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(BAMPFA_SOURCE_URL_PREFIX),
                    Source.name.like("bampfa_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(BAMPFA_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Berkeley Art Museum and Pacific Film Archive calendar events. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument("--url", default=BAMPFA_CALENDAR_URL)
    ...
    parser.add_argument(
        ...
        help="Directory used by --save-html (default: data/html/bampfa).",
    )
    parser.add_argument(
        "--dump-text",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all BAMPFA DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.dump_text:
        ...
        print(f"Saved BAMPFA text dump to: {dump_path}")
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_bampfa_entries()
        print(
            "Deleted BAMPFA rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                ...
                parse_payload=lambda payload: parse_bampfa_events_html(html=payload, list_url=args.url),
                parser_name="bampfa",
                adapter_type="bampfa_calendar",
                parsed_label="rows",
