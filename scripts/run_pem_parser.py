...
from src.crawlers.adapters.pem import PEM_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.pem import load_pem_payload  # noqa: E402
from src.crawlers.adapters.pem import parse_pem_payload  # noqa: E402
...
PEM_SOURCE_URL_PREFIX = "https://www.pem.org/%"


def clear_pem_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(PEM_SOURCE_URL_PREFIX),
                    Source.name.like("pem_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(PEM_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Peabody Essex Museum event pages. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all PEM DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_pem_entries()
        print(
            "Deleted PEM rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="pem",
                source_url=PEM_EVENTS_URL,
                load_payload=load_pem_payload,
                parse_payload=parse_pem_payload,
                parser_name="run_pem_parser",
                adapter_type="pem_events",
                parsed_label="PEM rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Peabody Essex Museum"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_pem_parser",
            source_url=PEM_EVENTS_URL,
            details={"source": "Peabody Essex Museum"},
        ),
    )
