...
from src.crawlers.adapters.lsumoa import LSUMOA_PROGRAMS_URL  # noqa: E402
from src.crawlers.adapters.lsumoa import load_lsumoa_payload  # noqa: E402
from src.crawlers.adapters.lsumoa import parse_lsumoa_payload  # noqa: E402
...
LSUMOA_SOURCE_URL_PREFIX = "https://www.lsumoa.org/%"


def clear_lsumoa_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(LSUMOA_SOURCE_URL_PREFIX),
                    Source.name.in_(["www.lsumoa.org", "lsumoa_programs"]),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(LSUMOA_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse LSU Museum of Art museum programs. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument("--url", default=LSUMOA_PROGRAMS_URL)
    parser.add_argument(
        "--commit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all LSUMOA DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_lsumoa_entries()
        print(
            "Deleted LSUMOA rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    payload = await load_lsumoa_payload(list_url=args.url)
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                ...
                parse_payload=parse_lsumoa_payload,
                parser_name="run_lsumoa_parser",
                adapter_type="lsumoa_programs",
                parsed_label="lsumoa rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "LSU Museum of Art"},
            )
        ],
        commit=args.commit,
    )
