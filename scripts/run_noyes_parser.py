...
from src.crawlers.adapters.noyes import NOYES_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.noyes import load_noyes_events_payload  # noqa: E402
from src.crawlers.adapters.noyes import parse_noyes_events_payload  # noqa: E402
...
NOYES_SOURCE_URL_PREFIX = "https://noyesmuseum.org/%"


def clear_noyes_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [("Noyes Museum of Art", "Atlantic City", "NJ")],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(NOYES_SOURCE_URL_PREFIX),
                    Source.name.like("noyes_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(NOYES_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Noyes Museum of Art events. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Noyes DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_noyes_entries()
        print(
            "Deleted Noyes rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    run_specs = [
        TargetRunSpec(
            name="noyes_events",
            source_url=NOYES_EVENTS_URL,
            load_payload=load_noyes_events_payload,
            parse_payload=parse_noyes_events_payload,
            parser_name="run_noyes_parser",
            adapter_type="noyes_events",
            parsed_label="Noyes rows",
