...
from src.crawlers.adapters.academy_art_museum import (  # noqa: E402
    ACADEMY_ART_MUSEUM_EVENTS_URL,
    fetch_academy_art_museum_events_page,
    parse_academy_art_museum_events_html,
)
...
DEFAULT_CACHE_DIR = Path("data") / "html" / "academy_art_museum"
ACADEMY_SOURCE_URL_PREFIX = "https://academyartmuseum.org/%"


def _write_html_cache(html: str, cache_dir: Path) -> Path:
    cache_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    output_path = cache_dir / f"academy_art_museum_events_{stamp}.html"
...
def _write_text_dump(
    ...
    *,
    ...
    if source_html_path is not None:
        ...
        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        output_path = dump_dir / f"academy_art_museum_events_{stamp}.txt"
    ...
    *,
    ...
    if input_html:
        ...
        if not input_path.exists():
            ...
            raise SystemExit(1)
        print(f"Loading Academy Art Museum HTML from file: {input_path}")
        html = input_path.read_text(encoding="utf-8")
        return html, input_path

    try:
        html = await fetch_academy_art_museum_events_page(url)
    ...
    if save_html:
        ...
        print(f"Saved raw Academy Art Museum HTML cache to: {cache_path}")

    return html, None


def clear_academy_art_museum_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(ACADEMY_SOURCE_URL_PREFIX),
                    Source.name.like("academy_art_museum_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(ACADEMY_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Academy Art Museum classes and workshops events. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument("--url", default=ACADEMY_ART_MUSEUM_EVENTS_URL)
    ...
    parser.add_argument(
        ...
        help="Directory used by --save-html (default: data/html/academy_art_museum).",
    )
    parser.add_argument(
        "--dump-text",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Academy Art Museum DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_academy_art_museum_entries()
        print(
            "Deleted Academy Art Museum rows: "
    ...
    if args.dump_text:
        ...
        print(f"Saved Academy Art Museum text dump to: {dump_path}")
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_academy_art_museum_entries()
        print(
            "Deleted Academy Art Museum rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="academy_art_museum_events",
                source_url=args.url,
                load_payload=_load_cached_html,
                parse_payload=lambda payload: parse_academy_art_museum_events_html(
                    html=payload,
                    list_url=args.url,
                ),
                parser_name="academy_art_museum",
                adapter_type="academy_art_museum_events",
                parsed_label="rows",
