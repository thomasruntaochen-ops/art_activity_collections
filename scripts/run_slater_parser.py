...
from src.crawlers.adapters.slater import SLATER_EVENTS_ICS_URL  # noqa: E402
from src.crawlers.adapters.slater import fetch_slater_events_ics  # noqa: E402
from src.crawlers.adapters.slater import parse_slater_events_ics  # noqa: E402
...
SLATER_SOURCE_URL_PREFIX = "https://www.slatermuseum.org/%"


def clear_slater_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(SLATER_SOURCE_URL_PREFIX),
                    Source.name.like("slater_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(SLATER_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Slater Memorial Museum ICS events. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Slater DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_slater_entries()
        print(
            "Deleted Slater rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="slater_events",
                source_url=SLATER_EVENTS_ICS_URL,
                load_payload=fetch_slater_events_ics,
                parse_payload=lambda payload: parse_slater_events_ics(payload, start_date=args.start_date),
                parser_name="run_slater_parser",
                adapter_type="slater_events",
                parsed_label="Slater rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"start_date": args.start_date},
            )
        ],
        commit=args.commit,
