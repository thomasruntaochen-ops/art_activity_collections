...
from src.crawlers.adapters.mfah import (  # noqa: E402
    MFAH_EVENTS_URL,
    load_mfah_payload,
    parse_mfah_payload,
)
...
DEFAULT_CACHE_DIR = Path("data") / "html" / "mfah"
...
def _write_html_cache(payload: dict[str, object], cache_dir: Path) -> Path:
    ...
    output_path = cache_dir / f"mfah_events_{stamp}.json"
...
def _write_text_dump(payload: dict[str, object], dump_dir: Path) -> Path:
    ...
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    output_path = dump_dir / f"mfah_events_{stamp}.txt"
    ...
    *,
    ...
    if input_json:
        ...
        if not input_path.exists():
            ...
            raise SystemExit(1)
        print(f"Loading MFAH payload from file: {input_path}")
        return json.loads(input_path.read_text(encoding="utf-8"))

    try:
        payload = await load_mfah_payload(url)
    ...
    if save_html:
        ...
        print(f"Saved raw MFAH payload cache to: {cache_path}")
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Museum of Fine Arts, Houston family activities. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument("--url", default=MFAH_EVENTS_URL)
    ...
    parser.add_argument(
        ...
        help="Directory used by --save-html (default: data/html/mfah).",
    )
    parser.add_argument(
        "--dump-text",
        action="store_true",
    ...
    if args.dump_text:
        ...
        print(f"Saved MFAH text dump to: {dump_path}")

    parsed = parse_mfah_payload(payload)
