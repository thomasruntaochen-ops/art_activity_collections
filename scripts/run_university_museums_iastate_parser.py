...
IASTATE_SOURCE_URL_PREFIX = "https://museums.iastate.edu/%"


def clear_iastate_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.adapter_type == "university_museums_iastate_events",
                    Source.base_url.like("https://%museums.iastate.edu"),
                    Source.name.like("%museums.iastate.edu%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(IASTATE_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all University Museums ISU DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_iastate_entries()
        print(
            "Deleted University Museums ISU rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_iastate_entries()
        print(
            "Deleted University Museums ISU rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
