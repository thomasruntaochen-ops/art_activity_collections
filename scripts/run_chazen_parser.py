...
from src.crawlers.adapters.chazen import CHAZEN_VENUE_NAME  # noqa: E402
...
CHAZEN_SOURCE_URL_PREFIX = "https://chazen.wisc.edu/%"


def clear_chazen_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [(CHAZEN_VENUE_NAME, "Madison", "WI")])

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(CHAZEN_SOURCE_URL_PREFIX),
                    Source.name == "chazen_events",
                    Source.adapter_type == "chazen_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(CHAZEN_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_chazen_entries()
        print(
            "Deleted Chazen rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
