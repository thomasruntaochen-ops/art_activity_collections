...
from src.crawlers.adapters.harvard import HARVARD_CALENDAR_URL  # noqa: E402
from src.crawlers.adapters.harvard import load_harvard_payload  # noqa: E402
from src.crawlers.adapters.harvard import parse_harvard_payload  # noqa: E402
...
HARVARD_SOURCE_URL_PREFIX = "https://harvardartmuseums.org/%"


def clear_harvard_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(HARVARD_SOURCE_URL_PREFIX),
                    Source.name.like("harvard_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(HARVARD_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Harvard Art Museums calendar listing events. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Harvard DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_harvard_entries()
        print(
            "Deleted Harvard rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_harvard_entries()
        print(
            "Deleted Harvard rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="harvard",
                source_url=HARVARD_CALENDAR_URL,
                load_payload=load_harvard_payload,
                parse_payload=parse_harvard_payload,
                parser_name="run_harvard_parser",
                adapter_type="harvard_calendar_listings",
                parsed_label="Harvard rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Harvard Art Museums"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_harvard_parser",
            source_url=HARVARD_CALENDAR_URL,
            details={"source": "Harvard Art Museums"},
        ),
    )
