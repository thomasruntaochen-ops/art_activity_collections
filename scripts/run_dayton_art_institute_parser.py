...
from src.crawlers.adapters.dayton_art_institute import DAYTON_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.dayton_art_institute import load_dayton_art_institute_payload  # noqa: E402
from src.crawlers.adapters.dayton_art_institute import parse_dayton_art_institute_payload  # noqa: E402
...
def clear_dayton_art_institute_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [("Dayton Art Institute", "Dayton", "OH")])
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like("https://www.daytonartinstitute.org/%"),
                    Source.name == "dayton_art_institute_events",
                    Source.adapter_type == "dayton_art_institute_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like("https://www.daytonartinstitute.org/%")
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch and parse Dayton Art Institute upcoming activities.")
    parser.add_argument("--commit", action="store_true")
    parser.add_argument(
        "--clear",
        action="store_true",
        help=(
            "Delete all Dayton Art Institute DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_dayton_art_institute_entries()
        print(
            "Deleted Dayton Art Institute rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_dayton_art_institute_entries()
        print(
            "Deleted Dayton Art Institute rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="dayton_art_institute",
                source_url=DAYTON_EVENTS_URL,
                load_payload=lambda: asyncio.sleep(0, result=payload),
                parse_payload=parse_dayton_art_institute_payload,
                parser_name="run_dayton_art_institute_parser",
                adapter_type="dayton_art_institute_events",
                parsed_label="Dayton Art Institute rows",
