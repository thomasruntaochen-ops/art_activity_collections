...
from src.crawlers.adapters.ica_boston import ICA_BOSTON_CALENDAR_URL  # noqa: E402
from src.crawlers.adapters.ica_boston import load_ica_boston_calendar_payload  # noqa: E402
from src.crawlers.adapters.ica_boston import parse_ica_boston_calendar_payload  # noqa: E402
...
ICA_BOSTON_SOURCE_URL_PREFIX = "https://www.icaboston.org/%"


def clear_ica_boston_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(ICA_BOSTON_SOURCE_URL_PREFIX),
                    Source.name.like("ica_boston_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(ICA_BOSTON_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse ICA Boston calendar events. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all ICA Boston DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_ica_boston_entries()
        print(
            "Deleted ICA Boston rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_ica_boston_entries()
        print(
            "Deleted ICA Boston rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="ica_boston_calendar",
                source_url=ICA_BOSTON_CALENDAR_URL,
                load_payload=load_ica_boston_calendar_payload,
                parse_payload=parse_ica_boston_calendar_payload,
                parser_name="run_ica_boston_parser",
                adapter_type="ica_boston_calendar",
                parsed_label="ICA Boston rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Institute of Contemporary Art / Boston"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_ica_boston_parser",
            source_url=ICA_BOSTON_CALENDAR_URL,
            details={"source": "Institute of Contemporary Art / Boston"},
        ),
    )
