...
from src.crawlers.adapters.vt_bundle import VT_VENUES  # noqa: E402
from src.crawlers.adapters.vt_bundle import VT_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.vt_bundle import get_vt_source_prefixes  # noqa: E402
from src.crawlers.adapters.vt_bundle import load_vt_bundle_payload  # noqa: E402
from src.crawlers.adapters.vt_bundle import parse_vt_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Vermont museum activities from mixed official sources. "
            "Print parsed rows, and optionally commit to MySQL."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(VT_VENUES_BY_SLUG),
        default="all",
        help="Run all Vermont venues or one venue slug.",
    )
    parser.add_argument(
        "--start-date",
        default=None,
    ...
    for venue in selected_venues:
        ...
        targets.append(
            TargetRunSpec(
                name=f"vt_bundle_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload_by_slug.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_vt_events(
                    venue_payload,
                    venue=selected_venue,
                    start_date=args.start_date,
                ),
                parser_name="run_vt_bundle_parser",
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[vt-bundle] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any VT bundle venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed VT bundle with fetch failures: {failed}")
