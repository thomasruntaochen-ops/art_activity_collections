...
from src.crawlers.adapters.bruce import BRUCE_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.bruce import load_bruce_events_payload  # noqa: E402
from src.crawlers.adapters.bruce import parse_bruce_events_payload  # noqa: E402
...
BRUCE_SOURCE_URL_PREFIX = "https://brucemuseum.org/%"


def clear_bruce_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(BRUCE_SOURCE_URL_PREFIX),
                    Source.name.like("bruce_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(BRUCE_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Bruce Museum youth-oriented events. "
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional cap on fetched event listing pages.",
    )
    parser.add_argument(
        "--commit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Bruce Museum DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_bruce_entries()
        print(
            "Deleted Bruce Museum rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_bruce_entries()
        print(
            "Deleted Bruce Museum rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    run_specs = [
        TargetRunSpec(
            name="bruce_events",
            source_url=BRUCE_EVENTS_URL,
            load_payload=lambda: load_bruce_events_payload(page_limit=args.page_limit),
            parse_payload=parse_bruce_events_payload,
            parser_name="run_bruce_parser",
            adapter_type="bruce_events",
            parsed_label="Bruce rows",
