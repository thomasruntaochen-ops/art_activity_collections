...
from src.crawlers.adapters.rose import ROSE_PROGRAMS_URL  # noqa: E402
from src.crawlers.adapters.rose import load_rose_payload  # noqa: E402
from src.crawlers.adapters.rose import parse_rose_payload  # noqa: E402
...
ROSE_SOURCE_URL_PREFIX = "https://www.brandeis.edu/rose/%"


def clear_rose_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(ROSE_SOURCE_URL_PREFIX),
                    Source.name.like("rose_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(ROSE_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Rose Art Museum program pages. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Rose DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_rose_entries()
        print(
            "Deleted Rose rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="rose",
                source_url=ROSE_PROGRAMS_URL,
                load_payload=load_rose_payload,
                parse_payload=parse_rose_payload,
                parser_name="run_rose_parser",
                adapter_type="rose_programs",
                parsed_label="Rose rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Rose Art Museum"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_rose_parser",
            source_url=ROSE_PROGRAMS_URL,
            details={"source": "Rose Art Museum"},
        ),
    )
