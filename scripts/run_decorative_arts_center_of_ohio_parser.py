...
DECORATIVE_SOURCE_URL_PREFIX = "https://www.decartsohio.org/%"


def clear_decorative_arts_center_of_ohio_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [("Decorative Arts Center of Ohio", "Lancaster", "OH")])
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(DECORATIVE_SOURCE_URL_PREFIX),
                    Source.name.like("decorative_arts_center_of_ohio%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(DECORATIVE_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    ...
    parser.add_argument("--clear", action="store_true", help="Delete scoped Decorative Arts Center of Ohio rows before commit.")
    args = parser.parse_args()

    if args.clear and not args.commit:
        deleted = clear_decorative_arts_center_of_ohio_entries()
        print(
            "Deleted Decorative Arts Center of Ohio rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_decorative_arts_center_of_ohio_entries()
        print(
            "Deleted Decorative Arts Center of Ohio rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
