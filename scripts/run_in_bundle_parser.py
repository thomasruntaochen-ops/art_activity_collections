...
from src.crawlers.adapters.in_bundle import IN_VENUES  # noqa: E402
from src.crawlers.adapters.in_bundle import IN_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.in_bundle import get_in_source_prefixes  # noqa: E402
from src.crawlers.adapters.in_bundle import load_in_bundle_payload  # noqa: E402
from src.crawlers.adapters.in_bundle import parse_in_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Indiana museum activities from Eiteljorg, Eskenazi, "
            "Fort Wayne Museum of Art, Raclin Murphy Museum of Art, and South Bend Museum of Art."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(IN_VENUES_BY_SLUG),
        default="all",
        help="Run all Indiana venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Indiana bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    payload = await load_in_bundle_payload(venues=selected_venues)
    ...
    for venue in selected_venues:
        ...
        targets.append(
            TargetRunSpec(
                name=f"in_bundle_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload_by_slug.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_in_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_in_bundle_parser",
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[in-bundle] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any IN bundle venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed IN bundle with fetch failures: {failed}")
