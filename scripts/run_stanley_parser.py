...
STANLEY_SOURCE_URL_PREFIX = "https://stanleymuseum.uiowa.edu/%"


def clear_stanley_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(STANLEY_SOURCE_URL_PREFIX),
                    Source.name.like("stanley%"),
                    Source.adapter_type == "stanley_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(STANLEY_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Stanley Museum DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_stanley_entries()
        print(
            "Deleted Stanley rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_stanley_entries()
        print(
            "Deleted Stanley rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
