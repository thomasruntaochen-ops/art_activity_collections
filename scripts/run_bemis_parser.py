...
BEMIS_SOURCE_URL_PREFIX = "https://www.bemiscenter.org/%"


def clear_bemis_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(db, [("Bemis Center for Contemporary Arts", "Omaha", "NE")])

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(BEMIS_SOURCE_URL_PREFIX),
                    Source.name == "bemis_events",
                    Source.adapter_type == "bemis_events",
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(BEMIS_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_bemis_entries()
        print(
            "Deleted Bemis rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
