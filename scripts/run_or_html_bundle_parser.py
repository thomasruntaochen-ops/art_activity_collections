...
from src.crawlers.adapters.or_html_bundle import OR_HTML_VENUES  # noqa: E402
from src.crawlers.adapters.or_html_bundle import OR_HTML_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.or_html_bundle import get_or_html_source_prefixes  # noqa: E402
from src.crawlers.adapters.or_html_bundle import load_or_html_bundle_payload  # noqa: E402
from src.crawlers.adapters.or_html_bundle import parse_or_html_events  # noqa: E402
...
OR_HTML_SOURCE_URL_PREFIXES = get_or_html_source_prefixes()


def clear_or_html_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [(venue.venue_name, venue.city, venue.state) for venue in OR_HTML_VENUES],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.in_([venue.list_url for venue in OR_HTML_VENUES]),
                    Source.name.like("or_html_%"),
                    Source.name.in_([venue.source_name for venue in OR_HTML_VENUES]),
                )
            )
        ).all()

        url_filters = [Activity.source_url.like(f"{prefix}%") for prefix in OR_HTML_SOURCE_URL_PREFIXES]
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Oregon museums using HTML/detail-page parsing. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(OR_HTML_VENUES_BY_SLUG),
        default="all",
        help="Run all OR HTML venues or one venue slug.",
    )
    parser.add_argument(
        "--commit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all OR HTML bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    selected_venues = (
        list(OR_HTML_VENUES)
        if args.venue == "all"
        else [OR_HTML_VENUES_BY_SLUG[args.venue]]
    )

    if args.clear and not args.commit:
        deleted = clear_or_html_entries()
        print(
            "Deleted OR HTML rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_or_html_entries()
        print(
            "Deleted OR HTML rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    for venue in selected_venues:
        targets.append(
            TargetRunSpec(
                name=f"or_html_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_or_html_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_or_html_bundle_parser",
