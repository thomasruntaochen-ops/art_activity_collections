...
def clear_az_tribe_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [(venue.venue_name, venue.city, venue.state) for venue in AZ_TRIBE_VENUES],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.in_([venue.list_url for venue in AZ_TRIBE_VENUES]),
                    Source.name.like("az_%_events"),
                    Source.name.in_([venue.source_name for venue in AZ_TRIBE_VENUES]),
                )
            )
        ).all()

        url_filters = [Activity.source_url.like(f"{prefix}%") for prefix in AZ_TRIBE_SOURCE_URL_PREFIXES]
...
async def main() -> None:
    ...
    if args.clear and not args.commit:
        deleted = clear_az_tribe_entries()
        print(
