...
from src.crawlers.adapters.az_tribe_bundle import AZ_TRIBE_VENUES  # noqa: E402
from src.crawlers.adapters.az_tribe_bundle import AZ_TRIBE_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.az_tribe_bundle import get_az_tribe_source_prefixes  # noqa: E402
from src.crawlers.adapters.az_tribe_bundle import load_az_tribe_bundle_payload  # noqa: E402
from src.crawlers.adapters.az_tribe_bundle import parse_az_tribe_events  # noqa: E402
...
AZ_TRIBE_SOURCE_URL_PREFIXES = get_az_tribe_source_prefixes()


def clear_az_tribe_entries(*, venues=None) -> dict[str, int]:
    deleted_activity_tags = 0
    deleted_activities = 0
    deleted_ingestion_runs = 0
    deleted_sources = 0
    selected_venues = list(venues) if venues is not None else list(AZ_TRIBE_VENUES)
    ...
    with SessionLocal() as db:
        ...
        if venues is None:
            source_ids = list(
                dict.fromkeys(source_ids + db.scalars(select(Source.id).where(Source.name.like("az_%_events"))).all())
            )

        source_url_prefixes = get_az_tribe_source_prefixes(selected_venues) or AZ_TRIBE_SOURCE_URL_PREFIXES
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Arizona museums using Tribe events APIs. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(AZ_TRIBE_VENUES_BY_SLUG),
        default="all",
        help="Run all AZ Tribe venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all AZ Tribe bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
            "If used without --commit, the script exits after deletion."
        ),
    )
    args = parser.parse_args()

    selected_venues = list(AZ_TRIBE_VENUES) if args.venue == "all" else [AZ_TRIBE_VENUES_BY_SLUG[args.venue]]

    if args.clear and not args.commit:
        deleted = clear_az_tribe_entries(venues=selected_venues)
        print(
            "Deleted AZ Tribe rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_az_tribe_entries(venues=selected_venues)
        print(
            "Deleted AZ Tribe rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    payload = await load_az_tribe_bundle_payload(
    ...
    for venue in selected_venues:
        ...
        targets.append(
            TargetRunSpec(
                name=f"az_tribe_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload_by_slug.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_az_tribe_events(
                    venue_payload,
                    venue=selected_venue,
                ),
                parser_name="run_az_tribe_bundle_parser",
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[az-tribe-fetch] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any AZ Tribe venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed AZ Tribe bundle with fetch failures: {failed}")
