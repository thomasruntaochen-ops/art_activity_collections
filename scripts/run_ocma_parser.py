...
from src.crawlers.adapters.ocma import (  # noqa: E402
    OCMA_CALENDAR_URL,
    fetch_ocma_calendar_page,
    parse_ocma_events_html,
)
...
def _write_html_cache(html: str, cache_dir: Path) -> Path:
    ...
    output_path = cache_dir / f"ocma_calendar_{stamp}.html"
...
def _write_text_dump(
    ...
    *,
    ...
    if source_html_path is not None:
        ...
        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        output_path = dump_dir / f"ocma_calendar_{stamp}.txt"
    ...
    *,
    ...
    if input_html:
        ...
        if not input_path.exists():
            ...
            raise SystemExit(1)
        print(f"Loading OCMA HTML from file: {input_path}")
        html = input_path.read_text(encoding="utf-8")
        return html, input_path

    try:
        html = await fetch_ocma_calendar_page(url)
    ...
    if save_html:
        ...
        print(f"Saved raw OCMA HTML cache to: {cache_path}")

    return html, None


def clear_ocma_entries() -> dict[str, int]:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Orange County Museum of Art calendar events. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument("--url", default=OCMA_CALENDAR_URL)
    ...
    parser.add_argument(
        ...
        help="Directory used by --save-html (default: data/html/ocma).",
    )
    parser.add_argument(
        "--dump-text",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all OCMA DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.dump_text:
        ...
        print(f"Saved OCMA text dump to: {dump_path}")
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_ocma_entries()
        print(
            "Deleted OCMA rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                ...
                parse_payload=lambda payload: parse_ocma_events_html(html=payload, list_url=args.url),
                parser_name="ocma",
                adapter_type="ocma_calendar",
                parsed_label="rows",
