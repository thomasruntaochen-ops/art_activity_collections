...
TOLEDO_SOURCE_URL_PREFIXES = (
    "https://toledomuseum.org/events%",
    "https://tma.ticketapp.org/portal/product/%",
)


def clear_toledo_museum_of_art_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like("https://toledomuseum.org/events%"),
                    Source.name == "toledo_museum_of_art_events",
                    Source.adapter_type == "toledo_museum_of_art_events",
                )
            )
        ).all()

        activity_filter = or_(*(Activity.source_url.like(prefix) for prefix in TOLEDO_SOURCE_URL_PREFIXES))
        if source_ids:
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Toledo Museum of Art DB rows (activity_tags, activities, "
            "ingestion_runs, sources). If used without --commit, the script exits after deletion."
        ),
    )
    args = parser.parse_args()

    if args.clear and not args.commit:
        deleted = clear_toledo_museum_of_art_entries()
        print(
            "Deleted Toledo Museum of Art rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_toledo_museum_of_art_entries()
        print(
            "Deleted Toledo Museum of Art rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
