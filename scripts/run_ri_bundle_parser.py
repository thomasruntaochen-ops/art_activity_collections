...
def clear_ri_bundle_entries(venues=None) -> dict[str, int]:
    deleted_activity_tags = 0
    deleted_activities = 0
    deleted_ingestion_runs = 0
    deleted_sources = 0
    selected_venues = list(venues) if venues is not None else list(RI_VENUES)
    source_prefixes = get_ri_source_prefixes(selected_venues)
    source_conditions = [
    ...
    if len(selected_venues) == len(RI_VENUES):
        source_conditions.append(Source.name.like("ri_%_events"))
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete selected RI bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_ri_bundle_entries(selected_venues)
        print(
