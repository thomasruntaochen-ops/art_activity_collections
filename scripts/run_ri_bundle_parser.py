...
from src.crawlers.adapters.ri_bundle import RI_VENUES  # noqa: E402
from src.crawlers.adapters.ri_bundle import RI_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.ri_bundle import get_ri_source_prefixes  # noqa: E402
from src.crawlers.adapters.ri_bundle import load_ri_bundle_payload  # noqa: E402
from src.crawlers.adapters.ri_bundle import parse_ri_events  # noqa: E402
...
RI_SOURCE_URL_PREFIXES = get_ri_source_prefixes()


def clear_ri_bundle_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        venue_ids = lookup_venue_ids(
            db,
            [(venue.venue_name, venue.city, venue.state) for venue in RI_VENUES],
        )

        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.in_([venue.list_url for venue in RI_VENUES]),
                    Source.name.like("ri_%_events"),
                    Source.name.in_([venue.source_name for venue in RI_VENUES]),
                )
            )
        ).all()

        url_filters = [Activity.source_url.like(f"{prefix}%") for prefix in RI_SOURCE_URL_PREFIXES]
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Rhode Island museum activities from venue-specific HTML sources. "
            "Print parsed rows, and optionally commit to MySQL."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(RI_VENUES_BY_SLUG),
        default="all",
        help="Run all Rhode Island venues or one venue slug.",
    )
    parser.add_argument(
        "--start-date",
        default=None,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="How many RISD calendar months to fetch, including the current month.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all RI bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
            "If used without --commit, the script exits after deletion."
        ),
    )
    args = parser.parse_args()

    selected_venues = list(RI_VENUES) if args.venue == "all" else [RI_VENUES_BY_SLUG[args.venue]]

    if args.clear and not args.commit:
        deleted = clear_ri_bundle_entries()
        print(
            "Deleted RI bundle rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_ri_bundle_entries()
        print(
            "Deleted RI bundle rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    for venue in selected_venues:
        ...
        targets.append(
            TargetRunSpec(
                name=f"ri_bundle_{venue.slug}",
                source_url=venue.list_url,
                load_payload=lambda slug=venue.slug: asyncio.sleep(0, result=payload_by_slug.get(slug) or {}),
                parse_payload=lambda venue_payload, selected_venue=venue: parse_ri_events(
                    venue_payload,
                    venue=selected_venue,
                    start_date=args.start_date,
                ),
                parser_name="run_ri_bundle_parser",
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[ri-bundle] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any RI bundle venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed RI bundle with fetch failures: {failed}")
