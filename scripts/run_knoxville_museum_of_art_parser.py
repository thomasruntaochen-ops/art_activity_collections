...
from src.crawlers.adapters.knoxville_museum_of_art import KNOXVILLE_EVENTS_URL  # noqa: E402
from src.crawlers.adapters.knoxville_museum_of_art import load_knoxville_payload  # noqa: E402
from src.crawlers.adapters.knoxville_museum_of_art import parse_knoxville_payload  # noqa: E402
...
KNOXVILLE_SOURCE_URL_PREFIX = "https://knoxart.org/%"


def clear_knoxville_entries() -> dict[str, int]:
    ...
    with SessionLocal() as db:
        ...
        source_ids = db.scalars(
            select(Source.id).where(
                or_(
                    Source.base_url.like(KNOXVILLE_SOURCE_URL_PREFIX),
                    Source.name.like("knoxville_museum_of_art_%"),
                )
            )
        ).all()

        activity_filter = Activity.source_url.like(KNOXVILLE_SOURCE_URL_PREFIX)
        if source_ids:
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Knoxville Museum of Art event API. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all Knoxville Museum of Art DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if args.clear and not args.commit:
        deleted = clear_knoxville_entries()
        print(
            "Deleted Knoxville Museum of Art rows: "
    ...
    def _clear_before_commit() -> None:
        ...
        deleted = clear_knoxville_entries()
        print(
            "Deleted Knoxville Museum of Art rows before repopulation: "
            f"activity_tags={deleted['activity_tags']}, "
            f"activities={deleted['activities']}, "
            f"ingestion_runs={deleted['ingestion_runs']}, "
            f"sources={deleted['sources']}"
    ...
    summary = await run_targets(
        targets=[
            TargetRunSpec(
                name="knoxville_museum_of_art",
                source_url=KNOXVILLE_EVENTS_URL,
                load_payload=lambda: load_knoxville_payload(page_limit=args.page_limit),
                parse_payload=parse_knoxville_payload,
                parser_name="run_knoxville_museum_of_art_parser",
                adapter_type="knoxville_museum_of_art_events",
                parsed_label="Knoxville Museum of Art rows",
                before_commit=_clear_before_commit if args.clear else None,
                empty_parse_details={"source": "Knoxville Museum of Art"},
            )
        ],
        commit=args.commit,
        empty_commit_guard=EmptyCommitGuard(
            parser_name="run_knoxville_museum_of_art_parser",
            source_url=KNOXVILLE_EVENTS_URL,
            details={"source": "Knoxville Museum of Art"},
        ),
    )
