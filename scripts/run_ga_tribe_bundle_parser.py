...
from src.crawlers.adapters.ga_tribe_bundle import GA_TRIBE_VENUES  # noqa: E402
from src.crawlers.adapters.ga_tribe_bundle import GA_TRIBE_VENUES_BY_SLUG  # noqa: E402
from src.crawlers.adapters.ga_tribe_bundle import get_ga_tribe_source_prefixes  # noqa: E402
from src.crawlers.adapters.ga_tribe_bundle import load_ga_tribe_bundle_payload  # noqa: E402
from src.crawlers.adapters.ga_tribe_bundle import parse_ga_tribe_events  # noqa: E402
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and parse Georgia museums using Tribe events APIs. "
            "Print parsed rows, and optionally commit to DB."
        )
    )
    parser.add_argument(
        "--venue",
        choices=["all"] + sorted(GA_TRIBE_VENUES_BY_SLUG),
        default="all",
        help="Run all GA Tribe venues or one venue slug.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete all GA Tribe bundle DB rows (activity_tags, activities, ingestion_runs, sources). "
    ...
    if errors_by_slug:
        for slug, message in sorted(errors_by_slug.items()):
            print(f"[ga-tribe-fetch] skipped venue={slug}: {message}")

    if not targets:
        failed = ", ".join(f"{slug}: {message}" for slug, message in sorted(errors_by_slug.items()))
        raise RuntimeError(f"Unable to fetch any GA Tribe venues. Failures: {failed}")
    ...
    if errors_by_slug:
        ...
        raise SystemExit(f"Completed GA Tribe bundle with fetch failures: {failed}")
