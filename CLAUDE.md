...
- For local MySQL batch runs, use `source ~/.zshrc && ./scripts/run_crawler_local.sh {batch_id}`. For Railway-targeting Mac runs, use `source ~/.zshrc && ./scripts/run_crawler_mac.sh {batch_id}`. `config/crawler_venues.toml` controls which venues run in each batch.
